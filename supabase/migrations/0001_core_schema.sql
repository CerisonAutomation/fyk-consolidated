-- =============================================================================
-- FYK Consolidated — 0001_core_schema.sql
-- Core schema: enums, profiles, photos, social graph, messaging, events,
-- board and safety.
--
-- Apply as the FIRST migration. Idempotent: safe to re-run.
--
-- Every table is created with RLS ENABLED and NO POLICIES, which means it is
-- deny-all until 0003_security_rls.sql runs. That ordering is deliberate: there is no
-- window where data is exposed. Storage policies depend on 0004_functions.sql,
-- so the file order in this directory is load-bearing and documented in
