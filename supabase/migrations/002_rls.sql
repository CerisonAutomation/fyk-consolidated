...
--   * `premium_entitlements` has NO client write policy — Plus cannot be self-granted.
...
-- ------------------------------------------- entitlements & audit log ------
-- SELECT only. No INSERT/UPDATE/DELETE policy exists, so `authenticated`
-- cannot grant itself Plus. Writes must come from service-role or SQL.
...
-- 1. New auth user -> profile row + free entitlement.
...
begin
  ...
  insert into public.premium_entitlements (profile_id) values (new.id) on conflict (profile_id) do nothing;
  return new;
end $$;
...
select 'entitlements writable' as leak, count(*) from pg_policies
  where schemaname='public' and tablename='premium_entitlements' and cmd <> 'SELECT'
union all
