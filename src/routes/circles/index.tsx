...
/**
 * Circles — reads `/api/groups`.
 *
 * Groups you can join, with members and proximity.
 *
 * This screen used to fetch `/api/circles`, a path built from its own
...
function CirclesScreen() {
  ...
  const { data, isLoading, error, refetch } = useScreenData("circles", {
    search,
    filter,
  });

  const act = useScreenAction("circles", "primary", {
    invalidate: ["screen", "circles"],
    onDone: () => vibrate(20),
    onRefused: setActionNote,
  });
  const actLabel = screenActionLabel("circles", "primary") ?? "Join";
    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      ...
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-[14px] text-zinc-500">Groups you can join, with members and proximity</p>
      ...
      {data?.items.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-zinc-200 bg-zinc-50 p-12 text-center">
          <p className="text-[14px] font-medium text-zinc-700">No circles yet</p>
          <p className="mt-1 text-[12px] text-zinc-500">Reads /api/groups</p>
        </div>
      ) : (
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">Reads /api/groups • writes Join. Refusals are shown as the server words them.</p>
      </div>
    </div>
  );
}
