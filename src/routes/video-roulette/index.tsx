...
function VideoRouletteScreen() {
  const { data, isLoading } = useQuery({
    queryKey: ["video-roulette"],
    queryFn: async () => ({
      items: Array.from({ length: 6 }, (_, i) => ({
        id: `${"video-roulette"}-${i}`,
        title: `${"video-roulette"} ${i + 1}`,
        description: "Video roulette with random matching, WebRTC",
      })),
    }),
  });
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-[14px] text-zinc-500">Video roulette with random matching, WebRTC</p>
      </div>
