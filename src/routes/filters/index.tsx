...
/**
 * Saved filters — reads `/api/discover/saved-searches`.
 *
 * Search presets you kept, up to ten.
 *
 * This screen used to fetch `/api/filters`, a path built from its own
...
function FiltersScreen() {
  ...
  const { data, isLoading, error, refetch } = useScreenData("filters", {
    search,
    filter,
  });

  const act = useScreenAction("filters", "primary", {
    invalidate: ["screen", "filters"],
    onDone: () => vibrate(20),
    onRefused: setActionNote,
  });
  const actLabel = screenActionLabel("filters", "primary") ?? "Delete";
    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      ...
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-[24px] font-bold tracking-tight text-black capitalize">Saved filters</h1>
            <p className="mt-1 text-[14px] text-zinc-500">Search presets you kept, up to ten</p>
      ...
      {data?.items.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-zinc-200 bg-zinc-50 p-12 text-center">
          <p className="text-[14px] font-medium text-zinc-700">No saved filters yet</p>
          <p className="mt-1 text-[12px] text-zinc-500">Reads /api/discover/saved-searches</p>
        </div>
      ) : (
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">Reads /api/discover/saved-searches • writes Delete. Refusals are shown as the server words them.</p>
      </div>
    </div>
  );
}
