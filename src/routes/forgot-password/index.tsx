  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => ({
      items: Array.from({ length: 6 }, (_, i) => ({
        id: `${"forgot-password"}-${i}`,
        title: `${"forgot-password"} ${i + 1}`,
        description: "Forgot password with email reset",
      })),
    }),
  });
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-[14px] text-zinc-500">Forgot password with email reset</p>
      </div>
