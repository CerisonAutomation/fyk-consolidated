...
/**
 * Profile insights — reads `/api/profile/analytics`.
 *
 * Who viewed your profile, and when.
 *
 * This screen used to fetch `/api/insights`, a path built from its own
...
function InsightsScreen() {
  ...
  const { data, isLoading, error, refetch } = useScreenData("profileInsights", {
    search,
    filter,
  });
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <p className="mb-3 text-[12px] text-zinc-500">Analytics are a record, not a to-do list.</p>
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-[14px] text-zinc-500">Who viewed your profile, and when</p>
      ...
      {data?.items.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-zinc-200 bg-zinc-50 p-12 text-center">
          <p className="text-[14px] font-medium text-zinc-700">No profile insights yet</p>
          <p className="mt-1 text-[12px] text-zinc-500">Reads /api/profile/analytics</p>
        </div>
      ) : (
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">Reads /api/profile/analytics • nothing to write from here. Refusals are shown as the server words them.</p>
      </div>
    </div>
  );
}
