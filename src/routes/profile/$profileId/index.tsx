...
// ─── Inline: Online Status Indicator ─────────────────────────────────────────
...
function SectionHeader({ label }: { label: string }) {
	return (
		<p
			className="font-mono uppercase tracking-[0.25em] text-gold mb-3"
			style={{ fontSize: "10px" }}
		>
			{label}
		</p>
...
function SocialLinkItem({
	...
	const color = colors[platform] || "var(--accent-primary)";
	return (
		<div className="flex items-center gap-3 py-2">
			...
			<div className="flex-1 min-w-0">
				...
				<p className="text-xs text-foreground/80 truncate">{handle}</p>
			</div>
			...
			>
				{medias.map((media, i) => (
					...
					>
						...
						/>
						<span className="absolute right-4 top-4 flex items-center gap-1.5 rounded-full border border-white/15 bg-black/40 px-3 py-2 text-[10px] font-semibold uppercase tracking-widest text-white backdrop-blur-xl transition group-hover:bg-black/60">
...
function ProfilePage() {
	...
	const handleFavoriteToggle = useCallback(() => {
		...
		if (newState) {
			addFavorite.mutate({ profileId: numericId });
		} else {
			removeFavorite.mutate({ profileId: numericId });
		}
	}, [p, isFav, numericId, addFavorite, removeFavorite]);
			...
			<div className="relative h-screen overflow-hidden bg-background">
				<div className="h-full overflow-y-auto overscroll-contain">
					<main className="relative mx-auto min-h-[calc(100vh-4rem)] w-full max-w-2xl">
						{/* Skeleton: top bar */}
						<div className="sticky top-0 z-30 flex items-center justify-between px-4 py-3 bg-background/80 backdrop-blur-xl">
						...
						{/* Skeleton: image */}
						<div className="w-full aspect-[3/4] bg-muted/30 animate-pulse" />
						{/* Skeleton: content */}
	...
	return (
		<div className="relative h-screen overflow-hidden bg-background">
			<div className="h-full overflow-y-auto overscroll-contain">
				<main className="relative mx-auto min-h-[calc(100vh-4rem)] w-full max-w-2xl">
					...
					<div className="relative -mt-24 z-10">
						<div className="glass-card rounded-t-3xl border-t border-white/10 px-5 pt-6 pb-32">
							{/* Name, age, verified, online */}
							<div className="flex items-start justify-between">
								<div className="flex-1 min-w-0">
									<div className="flex items-center gap-2 flex-wrap">
										<h1
											className="font-display text-3xl text-foreground truncate"
											style={{ lineHeight: 1.1 }}
										>
											{p.displayName ?? "Anonymous"}
										</h1>
										{p.isVerified && <VerifiedBadge size={20} />}
										{p.showAge !== false && p.age != null && (
							...
							{/* ── Stats Grid ── */}
							{(p.height || p.weight || p.bodyType || p.sexualPosition) && (
								<div className="mt-6">
											...
											<StatItem
												...
												icon={({ className }) => (
													<svg
														...
														strokeWidth="2"
													>
														<path d="M12 2v20M2 12h4M18 12h4M12 6l-2 2M12 6l2 2M12 18l-2-2M12 18l2-2" />
													</svg>
												)}
											/>
										)}
							...
							{tribes.length > 0 && (
								<div className="mt-6">
									<SectionHeader label="Tribes" />
									<div className="flex flex-wrap gap-1.5">
										{tribes.map((tribe) => (
											<PillTag key={tribe}>{tribe}</PillTag>
										))}
									</div>
								</div>
							)}
							...
							{lookingFor.length > 0 && (
								<div className="mt-6">
									...
									<div className="flex flex-wrap gap-1.5">
										{lookingFor.map((item) => (
											<PillTag key={item} color="var(--accent-secondary)">
												{item}
											</PillTag>
										))}
									</div>
			...
			{/* ── Top navigation bar (back, favorite, menu) ── */}
...
// ─── Inline: Section Label (for single-value rows) ───────────────────────────
