...
function PhotoVerificationScreen() {
  const { data, isLoading } = useQuery({
    queryKey: ["photo-verification"],
    queryFn: async () => ({
      items: Array.from({ length: 6 }, (_, i) => ({
        id: `${"photo-verification"}-${i}`,
        title: `${"photo-verification"} ${i + 1}`,
        description: "Photo verification with selfie, liveness, badge",
      })),
    }),
  });
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-[14px] text-zinc-500">Photo verification with selfie, liveness, badge</p>
      </div>
