...
/**
 * Emergency contacts — reads `/api/safety/contacts`.
 *
 * Trusted contacts for check-in and live location.
 *
 * This screen used to fetch `/api/emergency-contact`, a path built from its own
...
function EmergencyContactScreen() {
  ...
  const { data, isLoading, error, refetch } = useScreenData("emergencyContact", {
    search,
    filter,
  });

  const act = useScreenAction("emergencyContact", "primary", {
    invalidate: ["screen", "emergencyContact"],
    onDone: () => vibrate(20),
    onRefused: setActionNote,
  });
  const act2 = useScreenAction("emergencyContact", "secondary", {
    invalidate: ["screen", "emergencyContact"],
    onDone: () => vibrate(10),
    onRefused: setActionNote,
  });
  const actLabel = screenActionLabel("emergencyContact", "primary") ?? "Make default";
  const act2Label = screenActionLabel("emergencyContact", "secondary") ?? "Remove";
    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      ...
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-[24px] font-bold tracking-tight text-black capitalize">Emergency contacts</h1>
            <p className="mt-1 text-[14px] text-zinc-500">Trusted contacts for check-in and live location</p>
      ...
      {data?.items.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-zinc-200 bg-zinc-50 p-12 text-center">
          <p className="text-[14px] font-medium text-zinc-700">No emergency contacts yet</p>
          <p className="mt-1 text-[12px] text-zinc-500">Reads /api/safety/contacts</p>
        </div>
      ) : (
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">Reads /api/safety/contacts • writes Make default. Refusals are shown as the server words them.</p>
      </div>
    </div>
  );
}
