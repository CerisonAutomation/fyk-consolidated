...
function CompatibilityPage() {
  ...
  useEffect(() => {
    fetch("/api/discover/compatibility").then(r => r.json()).then(d => { setScores(d.scores ?? []); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 space-y-4">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">AI-powered compatibility — 5 dimensions, explainable, production with DB. Research: Rizz scores vibe, we do 5 dims.</p>
      </div>

      <div className="border rounded-xl p-4 bg-gradient-to-br from-purple-50 to-pink-50">
        <h3 className="font-semibold">💜 How It Works — Max Fidelity</h3>
        <ul className="text-xs text-muted-foreground mt-2 space-y-1 list-disc pl-4">
          <li><b>Interests:</b> tagOverlap Jaccard — shared interests / union</li>
          <li><b>Lifestyle:</b> lookingFor + intents alignment</li>
          <li><b>Communication:</b> reply rate, best time overlap</li>
          <li><b>Values:</b> tribes, relationship_status, ethnicity tolerance</li>
      ...
      <div className="space-y-2">
        {scores.slice(0, 10).map((s: any) => (
          <div key={s.id} className="border rounded-xl p-3">
            ...
            <div className="mt-2 grid grid-cols-5 gap-1 text-[10px]">
              {Object.entries(s.dimensions ?? {}).map(([k, v]: any) => (
                <div key={k} className="text-center">
                  <div className="w-full h-1 bg-muted rounded-full overflow-hidden"><div className="h-full bg-primary rounded-full" style={{ width: `${v}%` }} /></div>
        ...
        {scores.length === 0 && (
          <div className="border rounded-xl p-8 text-center">
            ...
            <div className="text-xs text-muted-foreground">Like profiles to generate scores — AI calculates 5 dimensions</div>
          </div>
        )}
      </div>
    </div>
