...
/**
 * Blocked users — reads `/api/social?view=blocks`.
 *
 * People you blocked; unblocking is immediate.
 *
 * This screen used to fetch `/api/blocked-users`, a path built from its own
...
function BlockedUsersScreen() {
  ...
  const { data, isLoading, error, refetch } = useScreenData("blockedUsers", {
    search,
    filter,
  });

  const act = useScreenAction("blockedUsers", "primary", {
    invalidate: ["screen", "blockedUsers"],
    onDone: () => vibrate(20),
    onRefused: setActionNote,
  });
  const actLabel = screenActionLabel("blockedUsers", "primary") ?? "Unblock";
    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      ...
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-[14px] text-zinc-500">People you blocked; unblocking is immediate</p>
      ...
      {data?.items.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-zinc-200 bg-zinc-50 p-12 text-center">
          <p className="text-[14px] font-medium text-zinc-700">No blocked users yet</p>
          <p className="mt-1 text-[12px] text-zinc-500">Reads /api/social?view=blocks</p>
        </div>
      ) : (
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">Reads /api/social?view=blocks • writes Unblock. Refusals are shown as the server words them.</p>
      </div>
    </div>
  );
}
