...
function HiddenUsersPage() {
	...
	const profileIds = hiddenUsers.map((u) => u.profileId);
	const { data: profiles = [] } = useProfiles(profileIds);
	const unhideUser = useUnhideUser();
			...
			{isLoading ? (
				...
				<div className="flex flex-col gap-2 p-4">
					{hiddenUsers.map((hidden) => {
						const profile = profileMap.get(hidden.profileId);
						const displayName =
							(profile?.displayName as string | null) ??
							`User #${hidden.profileId}`;
						const mediaHash =
