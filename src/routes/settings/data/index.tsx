...
// Data Settings — GDPR export delete, cache, offline queue, storage — MAX DEPTH MODE — canonical real working production code example on GitHub
// Features: Cache clear, size display • Offline queue with retry, telemetry • Storage usage, media, messages, cache • GDPR export JSON/ZIP • Data retention 90 days, auto-cleanup
...
function DataSettingsScreen() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["data-settings"] });
    },
  });
    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-[24px] font-bold tracking-tight text-black">Data Settings</h1>
            <p className="mt-1 text-[14px] text-zinc-500">GDPR export delete, cache, offline queue, storage</p>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-2">
          ...
          <div key={0} className="rounded-[12px] border border-black/[0.04] bg-zinc-50 p-3">
            <p className="text-[12px] font-medium text-black">Cache clear, size display</p>
            <p className="mt-0.5 text-[11px] text-zinc-500">Cache clear, size display</p>
          </div>
          <div key={1} className="rounded-[12px] border border-black/[0.04] bg-zinc-50 p-3">
            <p className="text-[12px] font-medium text-black">Offline queue with retry, telemetry</p>
            <p className="mt-0.5 text-[11px] text-zinc-500">Offline queue with retry, telemetry</p>
          </div>
          <div key={2} className="rounded-[12px] border border-black/[0.04] bg-zinc-50 p-3">
            <p className="text-[12px] font-medium text-black">Storage usage, media, messages, cache</p>
            <p className="mt-0.5 text-[11px] text-zinc-500">Storage usage, media, messages, cache</p>
          </div>
          <div key={3} className="rounded-[12px] border border-black/[0.04] bg-zinc-50 p-3">
            <p className="text-[12px] font-medium text-black">GDPR export JSON/ZIP</p>
            <p className="mt-0.5 text-[11px] text-zinc-500">GDPR export JSON/ZIP</p>
          </div>
          <div key={4} className="rounded-[12px] border border-black/[0.04] bg-zinc-50 p-3">
            <p className="text-[12px] font-medium text-black">Data retention 90 days, auto-cleanup</p>
            <p className="mt-0.5 text-[11px] text-zinc-500">Data retention 90 days, auto-cleanup</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-4">
          <div className="flex items-center justify-between rounded-[12px] border bg-zinc-50 p-3">
            <div>
              <p className="text-[13px] font-medium text-black">Enable Data Settings</p>
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">PRD 11/12/13/14 • data • Max depth • Canonical real working production code example on GitHub • Enterprise • No stubs • No fabricated</p>
      </div>
    </div>
  );
}
