  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => ({
      items: Array.from({ length: 6 }, (_, i) => ({
        id: `${"accessibility-settings"}-${i}`,
        title: `${"accessibility-settings"} ${i + 1}`,
        description: "Accessibility WCAG 2.2 AA with font size, contrast",
      })),
    }),
  });
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <h1 className="font-display text-[24px] font-bold tracking-tight text-black capitalize">accessibility settings</h1>
        <p className="mt-1 text-[14px] text-zinc-500">Accessibility WCAG 2.2 AA with font size, contrast</p>
      </div>
