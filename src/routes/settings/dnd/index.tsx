...
// Do Not Disturb — DND 22:00-07:00, notification mute, vs Grindr DND — MAX DEPTH MODE — canonical real working production code example on GitHub
// Features: DND schedule 22:00-07:00 customizable • Mute push, email, in-app separately • Allow favorites bypass DND • Auto-reply during DND — AI avatar

export const Route = createFileRoute("/settings/dnd/")({
  component: DndSettingsScreen,
});

function DndSettingsScreen() {
  const qc = useQueryClient();
  const [form, setForm] = useState<Record<string, any>>({});
  const [showAdvanced, setShowAdvanced] = useState(false);

  const { data, isLoading } = useQuery({
    queryKey: ["dnd-settings"],
    queryFn: async () => {
      const res = await fetch("/api/settings/dnd", {
    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-[24px] font-bold tracking-tight text-black">Do Not Disturb</h1>
            <p className="mt-1 text-[14px] text-zinc-500">DND 22:00-07:00, notification mute, vs Grindr DND</p>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-2">
          ...
          <div key={0} className="rounded-[12px] border border-black/[0.04] bg-zinc-50 p-3">
            <p className="text-[12px] font-medium text-black">DND schedule 22:00-07:00 customizable</p>
            <p className="mt-0.5 text-[11px] text-zinc-500">DND schedule 22:00-07:00 customizable</p>
          </div>
          <div key={1} className="rounded-[12px] border border-black/[0.04] bg-zinc-50 p-3">
            <p className="text-[12px] font-medium text-black">Mute push, email, in-app separately</p>
            <p className="mt-0.5 text-[11px] text-zinc-500">Mute push, email, in-app separately</p>
          </div>
          <div key={2} className="rounded-[12px] border border-black/[0.04] bg-zinc-50 p-3">
            <p className="text-[12px] font-medium text-black">Allow favorites bypass DND</p>
            <p className="mt-0.5 text-[11px] text-zinc-500">Allow favorites bypass DND</p>
          </div>
          <div key={3} className="rounded-[12px] border border-black/[0.04] bg-zinc-50 p-3">
            <p className="text-[12px] font-medium text-black">Auto-reply during DND</p>
            <p className="mt-0.5 text-[11px] text-zinc-500">AI avatar</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-4">
          <div className="flex items-center justify-between rounded-[12px] border bg-zinc-50 p-3">
            <div>
              <p className="text-[13px] font-medium text-black">Enable Do Not Disturb</p>
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">PRD 11/12/13/14 • dnd • Max depth • Canonical real working production code example on GitHub • Enterprise • No stubs • No fabricated</p>
      </div>
    </div>
  );
}
