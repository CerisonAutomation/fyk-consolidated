...
// Discreet Icon — App icon masking calculator/weather, vs Grindr discreet icon — MAX DEPTH MODE — canonical real working production code example on GitHub
// Features: Icon options: default, calculator, weather, notes • Name masking • Requires PIN to reveal • iOS/Android alternate icons

export const Route = createFileRoute("/settings/discreet-icon/")({
  component: DiscreetIconSettingsScreen,
});

function DiscreetIconSettingsScreen() {
  const qc = useQueryClient();
  const [form, setForm] = useState<Record<string, any>>({});
  const [showAdvanced, setShowAdvanced] = useState(false);

  const { data, isLoading } = useQuery({
    queryKey: ["discreet-icon-settings"],
    queryFn: async () => {
      const res = await fetch("/api/settings/discreet-icon", {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["discreet-icon-settings"] });
    },
  });
    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-[24px] font-bold tracking-tight text-black">Discreet Icon</h1>
            <p className="mt-1 text-[14px] text-zinc-500">App icon masking calculator/weather, vs Grindr discreet icon</p>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-2">
          ...
          <div key={0} className="rounded-[12px] border border-black/[0.04] bg-zinc-50 p-3">
            <p className="text-[12px] font-medium text-black">Icon options: default, calculator, weather, notes</p>
            <p className="mt-0.5 text-[11px] text-zinc-500">Icon options: default, calculator, weather, notes</p>
          </div>
          <div key={1} className="rounded-[12px] border border-black/[0.04] bg-zinc-50 p-3">
            <p className="text-[12px] font-medium text-black">Name masking</p>
            <p className="mt-0.5 text-[11px] text-zinc-500">Name masking</p>
          </div>
          <div key={2} className="rounded-[12px] border border-black/[0.04] bg-zinc-50 p-3">
            <p className="text-[12px] font-medium text-black">Requires PIN to reveal</p>
            <p className="mt-0.5 text-[11px] text-zinc-500">Requires PIN to reveal</p>
          </div>
          <div key={3} className="rounded-[12px] border border-black/[0.04] bg-zinc-50 p-3">
            <p className="text-[12px] font-medium text-black">iOS/Android alternate icons</p>
            <p className="mt-0.5 text-[11px] text-zinc-500">iOS/Android alternate icons</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-4">
          <div className="flex items-center justify-between rounded-[12px] border bg-zinc-50 p-3">
            <div>
              <p className="text-[13px] font-medium text-black">Enable Discreet Icon</p>
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">PRD 11/12/13/14 • discreet-icon • Max depth • Canonical real working production code example on GitHub • Enterprise • No stubs • No fabricated</p>
      </div>
    </div>
  );
}
