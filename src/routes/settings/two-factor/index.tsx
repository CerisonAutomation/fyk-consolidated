  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => ({
      items: Array.from({ length: 6 }, (_, i) => ({
        id: `${"two-factor-auth"}-${i}`,
        title: `${"two-factor-auth"} ${i + 1}`,
        description: "2FA with TOTP, backup codes, QR",
      })),
    }),
  });
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <h1 className="font-display text-[24px] font-bold tracking-tight text-black capitalize">two factor auth</h1>
        <p className="mt-1 text-[14px] text-zinc-500">2FA with TOTP, backup codes, QR</p>
      </div>
