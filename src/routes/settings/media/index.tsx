  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => ({
      items: Array.from({ length: 6 }, (_, i) => ({
        id: `${"media-settings"}-${i}`,
        title: `${"media-settings"} ${i + 1}`,
        description: "Media settings with auto-play, quality, cache",
      })),
    }),
  });
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <h1 className="font-display text-[24px] font-bold tracking-tight text-black capitalize">media settings</h1>
        <p className="mt-1 text-[14px] text-zinc-500">Media settings with auto-play, quality, cache</p>
      </div>
