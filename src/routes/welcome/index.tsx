...
/**
 * Welcome — reads `/api/growth/funnel`.
 *
 * Onboarding steps and what is left to finish.
 *
 * This screen used to fetch `/api/welcome`, a path built from its own
...
function WelcomeScreen() {
  ...
  const { data, isLoading, error, refetch } = useScreenData("welcome", {
    search,
    filter,
  });

  const act = useScreenAction("welcome", "primary", {
    invalidate: ["screen", "welcome"],
    onDone: () => vibrate(20),
    onRefused: setActionNote,
  });
  const actLabel = screenActionLabel("welcome", "primary") ?? "Mark done";
    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      ...
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-[14px] text-zinc-500">Onboarding steps and what is left to finish</p>
      ...
      {data?.items.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-zinc-200 bg-zinc-50 p-12 text-center">
          <p className="text-[14px] font-medium text-zinc-700">No welcome yet</p>
          <p className="mt-1 text-[12px] text-zinc-500">Reads /api/growth/funnel</p>
        </div>
      ) : (
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">Reads /api/growth/funnel • writes Mark done. Refusals are shown as the server words them.</p>
      </div>
    </div>
  );
}
