...
function ChatListPage() {
	...
	const { data, isPending, error, refetch, isFetching } = useQuery({
		...
		queryFn: () =>
			api.get<{ conversations: ConversationRow[] }>("conversations"),
	...
	const conversations = useMemo(() => {
		...
		return all.filter(
			(row) =>
				(row.other?.displayName ?? "").toLowerCase().includes(needle) ||
	...
	if (!capable("chat")) {
		return (
			<StateBlock
				...
				description="conversations and messages are not readable for your account. Apply the migrations in supabase/migrations and reload."
			/>
		);
	}
	...
	return (
		...
		>
			<div className="mb-3 flex items-center gap-2">
				<label className="relative flex-1">
					...
					<input
						...
						className="entry-input h-11 pl-10 text-[13.5px]"
					/>
				</label>
				<button
					type="button"
					onClick={() => void refetch()}
					className="press flex h-11 w-11 items-center justify-center rounded-full border border-line bg-surface text-ink-2"
			...
			{isPending ? (
				...
				<StateBlock
					kind="error"
					title="Your chats could not load"
				...
				<StateBlock
					...
					description={
						term
							...
							: "A chat opens when you and someone else tap each other. Nothing here is simulated: no bot will message you first."
					}
					action={
						<button
							type="button"
						...
						<li key={row.id}>
							...
							>
								<span className="relative shrink-0">
									<Avatar
										name={row.other?.displayName ?? "Former member"}
										photoUrl={row.other?.avatarUrl ?? null}
										size={46}
										online={row.other?.presence === "online"}
									/>
								</span>
								<span className="min-w-0 flex-1">
									<span className="flex items-baseline gap-2">
										<span
											className={
												row.unread > 0
													? "truncate text-[14.5px] font-bold text-ink"
													: "truncate text-[14.5px] font-semibold text-ink-2"
											}
										>
									...
									<span className="mt-0.5 flex items-center gap-2">
										...
										{row.unread > 0 ? (
											<span className="ml-auto shrink-0 rounded-full bg-gold px-1.5 text-[10.5px] font-bold leading-4 text-black">
												{row.unread}
											</span>
										) : null}
									</span>
