...
/**
 * Shout — reads `/api/shouts?id=`.
 *
 * One shout: content, author, likes.
 *
...
function ShoutIdScreen() {
  ...
  const { shoutId } = Route.useParams();
  const { data, isLoading, error, refetch } = useScreenData("shoutDetail", {
    search,
    filter, id: shoutId,
  });

  const act = useScreenAction("shoutDetail", "primary", {
    invalidate: ["screen", "shoutDetail"],
    onDone: () => vibrate(20),
    onRefused: setActionNote,
  });
  const act2 = useScreenAction("shoutDetail", "secondary", {
    invalidate: ["screen", "shoutDetail"],
    onDone: () => vibrate(10),
    onRefused: setActionNote,
  });
  const actLabel = screenActionLabel("shoutDetail", "primary") ?? "Like";
  const act2Label = screenActionLabel("shoutDetail", "secondary") ?? "Boost";
    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      ...
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-[24px] font-bold tracking-tight text-black capitalize">Shout</h1>
            <p className="mt-1 text-[14px] text-zinc-500">One shout: content, author, likes</p>
      ...
      {data?.items.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-zinc-200 bg-zinc-50 p-12 text-center">
          <p className="text-[14px] font-medium text-zinc-700">No shout yet</p>
          <p className="mt-1 text-[12px] text-zinc-500">Reads /api/shouts?id=</p>
        </div>
      ) : (
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">Reads /api/shouts?id= • writes Like. Refusals are shown as the server words them.</p>
      </div>
    </div>
  );
}
