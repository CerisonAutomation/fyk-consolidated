  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => ({
      items: Array.from({ length: 6 }, (_, i) => ({
        id: `${"report-user"}-${i}`,
        title: `${"report-user"} ${i + 1}`,
        description: "Report user with reason, description, screenshot",
      })),
    }),
  });
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-[14px] text-zinc-500">Report user with reason, description, screenshot</p>
      </div>
