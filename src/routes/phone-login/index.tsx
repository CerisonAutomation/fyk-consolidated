...
function PhoneLoginScreen() {
  const { data, isLoading } = useQuery({
    queryKey: ["phone-login"],
    queryFn: async () => ({
      items: Array.from({ length: 6 }, (_, i) => ({
        id: `${"phone-login"}-${i}`,
        title: `${"phone-login"} ${i + 1}`,
        description: "Phone login with OTP, verification",
      })),
    }),
  });
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-[14px] text-zinc-500">Phone login with OTP, verification</p>
      </div>
