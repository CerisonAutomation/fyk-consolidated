...
/**
 * Event — reads `/api/events?id=`.
 *
 * One event: who is going, where, and when.
 *
...
function EventIdScreen() {
  ...
  const { data, isLoading, error, refetch } = useScreenData("eventDetail", {
    search,
    filter, id: eventId,
  });

  const act = useScreenAction("eventDetail", "primary", {
    invalidate: ["screen", "eventDetail"],
    onDone: () => vibrate(20),
    onRefused: setActionNote,
  });
  const act2 = useScreenAction("eventDetail", "secondary", {
    invalidate: ["screen", "eventDetail"],
    onDone: () => vibrate(10),
    onRefused: setActionNote,
  });
  const actLabel = screenActionLabel("eventDetail", "primary") ?? "Going";
  const act2Label = screenActionLabel("eventDetail", "secondary") ?? "Not going";
    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      ...
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-[24px] font-bold tracking-tight text-black capitalize">Event</h1>
            <p className="mt-1 text-[14px] text-zinc-500">One event: who is going, where, and when</p>
      ...
      {data?.items.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-zinc-200 bg-zinc-50 p-12 text-center">
          <p className="text-[14px] font-medium text-zinc-700">No event yet</p>
          <p className="mt-1 text-[12px] text-zinc-500">Reads /api/events?id=</p>
        </div>
      ) : (
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">Reads /api/events?id= • writes Going. Refusals are shown as the server words them.</p>
      </div>
    </div>
  );
}
