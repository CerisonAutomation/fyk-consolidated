...
/**
 * Dump-rify — reads `/api/discover`.
 *
 * Keep or dump, one profile at a time.
 *
 * This screen used to fetch `/api/dump-rify`, a path built from its own
...
function DumpRifyScreen() {
  ...
  const { data, isLoading, error, refetch } = useScreenData("dumpRify", {
    search,
    filter,
  });

  const act = useScreenAction("dumpRify", "primary", {
    invalidate: ["screen", "dumpRify"],
    onDone: () => vibrate(20),
    onRefused: setActionNote,
  });
  const act2 = useScreenAction("dumpRify", "secondary", {
    invalidate: ["screen", "dumpRify"],
    onDone: () => vibrate(10),
    onRefused: setActionNote,
  });
  const actLabel = screenActionLabel("dumpRify", "primary") ?? "Keep";
  const act2Label = screenActionLabel("dumpRify", "secondary") ?? "Dump";
    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      ...
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-[14px] text-zinc-500">Keep or dump, one profile at a time</p>
      ...
      {data?.items.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-zinc-200 bg-zinc-50 p-12 text-center">
          <p className="text-[14px] font-medium text-zinc-700">No dump-rify yet</p>
          <p className="mt-1 text-[12px] text-zinc-500">Reads /api/discover</p>
        </div>
      ) : (
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">Reads /api/discover • writes Keep. Refusals are shown as the server words them.</p>
      </div>
    </div>
  );
}
