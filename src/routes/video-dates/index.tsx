...
/**
 * Video dates — reads `/api/calls`.
 *
 * Calls: ringing, accepted, ended.
 *
 * This screen used to fetch `/api/video-dates`, a path built from its own
...
function VideoDatesScreen() {
  ...
  const { data, isLoading, error, refetch } = useScreenData("videoDates", {
    search,
    filter,
  });

  const act = useScreenAction("videoDates", "primary", {
    invalidate: ["screen", "videoDates"],
    onDone: () => vibrate(20),
    onRefused: setActionNote,
  });
  const act2 = useScreenAction("videoDates", "secondary", {
    invalidate: ["screen", "videoDates"],
    onDone: () => vibrate(10),
    onRefused: setActionNote,
  });
  const actLabel = screenActionLabel("videoDates", "primary") ?? "Accept";
  const act2Label = screenActionLabel("videoDates", "secondary") ?? "Decline";
    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      ...
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-[14px] text-zinc-500">Calls: ringing, accepted, ended</p>
      ...
      {data?.items.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-zinc-200 bg-zinc-50 p-12 text-center">
          <p className="text-[14px] font-medium text-zinc-700">No video dates yet</p>
          <p className="mt-1 text-[12px] text-zinc-500">Reads /api/calls</p>
        </div>
      ) : (
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">Reads /api/calls • writes Accept. Refusals are shown as the server words them.</p>
      </div>
    </div>
  );
}
