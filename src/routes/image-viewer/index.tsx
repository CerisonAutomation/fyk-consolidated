...
/**
 * Albums — reads `/api/albums`.
 *
 * Your albums and the media in them.
 *
 * This screen used to fetch `/api/media`, a path built from its own
...
function ImageViewerScreen() {
  ...
  const { data, isLoading, error, refetch } = useScreenData("imageViewer", {
    search,
    filter,
  });
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <p className="mb-3 text-[12px] text-zinc-500">Open an album to view its media.</p>
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-[24px] font-bold tracking-tight text-black capitalize">Albums</h1>
            <p className="mt-1 text-[14px] text-zinc-500">Your albums and the media in them</p>
      ...
      {data?.items.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-zinc-200 bg-zinc-50 p-12 text-center">
          <p className="text-[14px] font-medium text-zinc-700">No albums yet</p>
          <p className="mt-1 text-[12px] text-zinc-500">Reads /api/albums</p>
        </div>
      ) : (
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">Reads /api/albums • nothing to write from here. Refusals are shown as the server words them.</p>
      </div>
    </div>
  );
}
