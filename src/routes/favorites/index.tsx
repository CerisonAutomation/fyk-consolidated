...
/**
 * Favourites — reads `/api/interest/favourites`.
 *
 * People you liked or vouched for.
 *
 * This screen used to fetch `/api/favorites`, a path built from its own
...
function FavoritesScreen() {
  ...
  const { data, isLoading, error, refetch } = useScreenData("favorites", {
    search,
    filter,
  });

  const act = useScreenAction("favorites", "primary", {
    invalidate: ["screen", "favorites"],
    onDone: () => vibrate(20),
    onRefused: setActionNote,
  });
  const actLabel = screenActionLabel("favorites", "primary") ?? "Remove";
    ...
    <div className="mx-auto max-w-3xl p-4 pb-24">
      ...
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-[24px] font-bold tracking-tight text-black capitalize">Favourites</h1>
            <p className="mt-1 text-[14px] text-zinc-500">People you liked or vouched for</p>
      ...
      {data?.items.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-zinc-200 bg-zinc-50 p-12 text-center">
          <p className="text-[14px] font-medium text-zinc-700">No favourites yet</p>
          <p className="mt-1 text-[12px] text-zinc-500">Reads /api/interest/favourites</p>
        </div>
      ) : (
      ...
      <div className="mt-6 rounded-[16px] border bg-zinc-50 p-4">
        <p className="text-[11px] text-zinc-500">Reads /api/interest/favourites • writes Remove. Refusals are shown as the server words them.</p>
      </div>
    </div>
  );
}
