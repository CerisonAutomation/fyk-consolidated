...
function PhotoEditorScreen() {
  const { data, isLoading } = useQuery({
    queryKey: ["photo-editor"],
    queryFn: async () => ({
      items: Array.from({ length: 6 }, (_, i) => ({
        id: `${"photo-editor"}-${i}`,
        title: `${"photo-editor"} ${i + 1}`,
        description: "Photo editor with crop, filter, blur, AI enhancer",
      })),
    }),
  });
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 pb-24">
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-[14px] text-zinc-500">Photo editor with crop, filter, blur, AI enhancer</p>
      </div>
