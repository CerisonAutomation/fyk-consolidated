...
import { CITY_CENTROIDS, compatibility, fuzzPin, pairHash, type LatLng } from "./geo";
import { normaliseRows } from "./gpu";
import { INTERESTS, KINKS, LIFESTYLE, LOOKING_FOR, TRIBES, tagOverlapScore } from "./taxonomy";
...
export const richPeople: RichPerson[] = people.map((p) => {
  ...
  const centre = CITY_CENTROIDS[p.city.toLowerCase()] ?? CITY_CENTROIDS.valletta!;
  const coords = fuzzPin({ lat: centre.lat, lng: centre.lng }, p.id, "grid", 2600);

  return {
    ...p,
    tagIds: Array.from(new Set([...lf, ...lfExtra, ...tribe, ...base, ...extraInterests, ...life])),
    kinkIds,
    coords,
    lastSeenMinutes: p.online ? 0 : Math.round(4 + pairHash(p.id, "seen") * 900),
    responseRate: 58 + Math.round(pairHash(p.id, "resp") * 40),
    joinedDaysAgo: 12 + Math.round(pairHash(p.id, "join") * 900),
    languages: LANGS[Math.floor(pairHash(p.id, "lang") * LANGS.length)] ?? ["English"],
  };
});
 ...
 */
const AREAS = ["Floriana", "Sliema", "Msida", "Gżira", "St Julian's", "Ta' Xbiex", "Pietà", "Marsa", "Birkirkara", "Ħamrun", "Qormi", "Mosta", "Naxxar", "Iklin", "Lija", "Paola", "Żabbar", "Mġarr", "Żejtun", "Valletta"];
const NAMES = ["Andre","Bruno","Caleb","Dean","Elias","Finn","Gabriel","Hugo","Isaac","Julien","Karim","Leo","Milo","Nico","Oscar","Pablo","Quentin","Rui","Sacha","Tomas","Uriel","Viktor","Wes","Xavi","Yannick","Zane","Aiden","Bastien","Cyrus","Dmitri","Emre","Fabio","Gustav","Hakan","Ivan","Joao","Kian","Lucas","Marco","Nadir"];
...
for (let i = 0; i < 228; i++) {
  ...
  const centre = CITY_CENTROIDS[base.city.toLowerCase()] ?? CITY_CENTROIDS.valletta!;
  derived.push({
    ...base,
    id,
    name,
...
export function findPerson(id: string | null | undefined): RichPerson | undefined {
...
export function profileVector(p: RichPerson, out: Float32Array, offset: number) {
  const text = `${p.name} ${p.area} ${p.headline} ${p.bio} ${[...p.tagIds, ...p.kinkIds].join(" ")}`.toLowerCase();
...
export function buildCorpus(): Float32Array<ArrayBuffer> {
  ...
  const data = new Float32Array(new ArrayBuffer(richPeople.length * SEARCH_DIM * 4));
...
export function scoreFor(
  ...
  const overlap = tagOverlapScore(myTags, [...person.tagIds, ...person.kinkIds]);
  const reciprocal = person.tagIds.some((t) => myTags.includes(t) && t.startsWith("chat"));
...
export const seedReports: ReportRow[] = [
  {
    ...
    details: "Profile text mentions being in year 12. Auto-hidden pending review.",
    status: "OPEN",
    priority: "URGENT",
    createdAt: "8 min ago",
  },
  {
    ...
    targetType: "MESSAGE",
    reason: "SCAM",
    details: "Asked three separate members to move to another app and send a gift card.",
    status: "OPEN",
    priority: "STANDARD",
    createdAt: "1 h ago",
  },
...
export const featureFlags = [
  { key: "ai.smart_replies", label: "Smart replies", enabled: true, note: "On-device encoder ranks the reply bank." },
  { key: "ai.translate", label: "Message translation", enabled: true, note: "Downloads one opus-mt pair on demand." },
  { key: "ai.plan_extract", label: "Plan detection", enabled: true, note: "Local date parser plus intent confirmation." },
  { key: "chat.calls", label: "Voice & video calls", enabled: true, note: "getUserMedia preview; signalling is stubbed." },
  { key: "discover.map", label: "Map view", enabled: true, note: "Leaflet with OpenStreetMap tiles." },
  { key: "voice.control", label: "Voice control", enabled: true, note: "Web Speech API, platform-provided." },
  { key: "safety.interstitials", label: "Safety interstitials", enabled: true, note: "Shown once per thread." },
  { key: "billing.dev_switch", label: "Local plan switch", enabled: true, note: "No payments are processed." },
];
