...
export function snap(point: LatLng, gridMeters = 250): LatLng {
	...
	const dLng =
		gridMeters / (111_320 * Math.max(0.2, Math.cos((lat * Math.PI) / 180)));
...
export function compatibility(input: {
	...
	const distanceScore =
		Math.max(0, 1 - input.distanceKm / Math.max(1, input.radiusKm)) * 24;
	const reciprocity = input.reciprocal ? 22 : 0;
	return Math.max(
		0,
		Math.min(
			100,
			Math.round(input.tagScore * 0.54 + distanceScore + reciprocity),
		),
	);
}
