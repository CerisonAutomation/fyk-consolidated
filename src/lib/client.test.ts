...
const envelope = <T,>(data: T) => ({ ok: true as const, data, requestId: "srv_1" });
...
describe("api", () => {
	it("sends cookies and a request id, and unwraps the envelope", async () => {
		...
		await expect(api.get<{ hello: string }>("discover")).resolves.toEqual({ hello: "world" });
		expect(calls()).toBe(1);
		expect(lastRequest?.url).toBe("/api/discover");
		expect(lastRequest?.init.credentials).toBe("include");
		expect(new Headers(lastRequest?.init.headers as HeadersInit).get("x-request-id")).toMatch(/^[a-z0-9_]{8,}$/i);
	});
	...
	it("throws a readable error built from the server envelope", async () => {
		stubFetch(() =>
			json(
				{ ok: false, error: { code: "rate_limited", message: "Slow down — try again in a minute." }, requestId: "srv_9" },
				{ status: 429, headers: { "retry-after": "31", "x-should-retry": "true" } },
			),
		);
		const error = await api.get("board").catch((caught) => caught as ApiClientError);
	...
	it("treats a conflict as final, not retryable", async () => {
		stubFetch(() => json({ ok: false, error: { code: "conflict", message: "You already have a live post." }, requestId: "r" }, { status: 409 }));
		const error = (await api.post("board", {}).catch((caught) => caught)) as ApiClientError;
		expect(error.retryable).toBe(false);
		expect(error.isBlocking).toBe(true);
	});

	it("flags an auth failure so the shell can re-check the session", async () => {
		stubFetch(() => json({ ok: false, error: { code: "unauthorized", message: "Sign in again." } }, { status: 401 }));
		const error = (await api.get("settings").catch((caught) => caught)) as ApiClientError;
		expect(error.isAuth).toBe(true);
	});

	it("retries a GET once on a network failure but never a POST", async () => {
		const getCount = stubFetch((call) => (call === 1 ? Promise.reject(new TypeError("fetch failed")) : json(envelope({ ok: true }))));
		await expect(api.get("notifications")).resolves.toEqual({ ok: true });
		expect(getCount()).toBe(2);

		const postCalls = stubFetch((call) => (call === 1 ? Promise.reject(new TypeError("fetch failed")) : json(envelope({ ok: true }))));
		await expect(api.post("messages", { body: "hi" })).rejects.toBeInstanceOf(ApiClientError);
	...
	it("sends a JSON body for writes and FormData untouched for uploads", async () => {
		...
		expect(new Headers(lastRequest?.init.headers as HeadersInit).get("content-type")).toContain("application/json");
		...
		expect(new Headers(lastRequest?.init.headers as HeadersInit).get("content-type")).toBeNull();
