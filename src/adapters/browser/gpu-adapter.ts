/**
 * Browser SimilarityEngine adapter.
 *
 * WebGPU compute path for similarity search. Ranking N profiles against
 * a query embedding is an embarrassingly parallel dot-product problem --
 * exactly what a GPU is for. Dispatches a WGSL compute shader over the
 * whole corpus in one pass and falls back to a tight typed-array loop
 * on the CPU when WebGPU is unavailable.
 *
 * Vectors are L2-normalised on the way in, so a dot product is the cosine.
 *
 * Implements the SimilarityEngine port from core/ports/services.ts.
 */

import type { SimilarityEngine } from "../../core/ports/services";
...
function normaliseRows(data: Float32Array, count: number, dim: number): void {
  for (let r = 0; r < count; r++) {
    const base = r * dim;
    let sum = 0;
...
class BrowserGpuEngine implements SimilarityEngine {
  private device: GPUDevice | null = null;
  private pipeline: GPUComputePipeline | null = null;
  private initTried = false;
  private initInfo: { available: boolean; backend: string } = {
    available: false,
    backend: "cpu",
  };
  ...
  async init(): Promise<{ available: boolean; backend: string }> {
    if (this.device) return this.initInfo;
    if (this.initTried && !this.device) return this.initInfo;
    this.initTried = true;

    if (!this.supported) {
      this.initInfo = { available: false, backend: "cpu" };
    ...
    try {
      ...
      this.initInfo = { available: true, backend: `webgpu (${name})` };
  ...
  /**
   * Returns one cosine score per row.
   * `corpus` is count x dim, row-major.
   * Falls back to CPU when the GPU pipeline is not ready.
   */
  async similarity(
    corpus: Float32Array,
    query: Float32Array,
    ...
    if (!device || !pipeline || count === 0) {
      return cpuSimilarity(corpus, query, count, dim);
    }

    const pad = (n: number) => Math.max(16, Math.ceil(n / 4) * 4);

    const corpusBuf = device.createBuffer({
      size: pad(corpus.byteLength),
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
    });
    const queryBuf = device.createBuffer({
      size: pad(query.byteLength),
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
    });
  ...
  /**
   * Times both paths over a synthetic corpus so the speed-up is
   * measured, not asserted.
   */
  async benchmark(
    count = 4096,
    dim = 384,
  ): Promise<{ gpuMs: number; cpuMs: number; speedup: number }> {
    ...
    if (this.device) {
      // warm-up
      await this.similarity(corpus, query, Math.min(count, 256), dim);
...
/** Singleton -- reuses the same GPU device across the application. */
export const browserGpuAdapter: SimilarityEngine = new BrowserGpuEngine();
