...
export function ChatView({
	...
	const [locationSheet, setLocationSheet] = useState<"share" | "pick" | null>(
		null,
	);
	...
	const { data: aiHealth } = useQuery({
		...
		queryFn: () =>
			api<{
				...
				method: "POST",
				body: { action: "chatHealth", conversationId },
			}),
		enabled: aiPanel,
	});
	...
	const react = useMutation({
		mutationFn: ({ messageId, emoji }: { messageId: string; emoji: string }) =>
			api(`/api/messages/${messageId}/react`, {
				method: "POST",
				body: { emoji },
			}),
		onSuccess: () =>
	...
	async function loadAI() {
		...
		try {
			const last =
				[...messages].reverse().find((m) => m.sender_id !== me?.id) ??
				messages[messages.length - 1];
			const r = await api<{ replies: string[]; intent: { label: string } }>(
				"/api/ai",
				{
					method: "POST",
					body: { action: "replies", message: last?.content ?? "" },
				},
			);
	...
	return (
		<div className="flex h-full flex-col">
			...
			<div className="flex items-center gap-3 border-b border-line pb-3">
				...
				<Avatar
					name={isGroup ? (conv.name ?? "Group") : (other?.pseudo ?? "")}
					photoUrl={isGroup ? null : other?.photos?.[0]}
					size={40}
					online={!isGroup && other?.online}
				/>
				<div className="min-w-0">
					<p className="truncate text-sm font-semibold text-white">
						{isGroup ? conv.name : other?.pseudo}
					</p>
				...
				<div className="border-b border-line py-2">
					...
					{search.trim() && (
						<p className="mt-1.5 px-1 text-[11px] text-muted">
							{filtered.length} matching messages
						</p>
					)}
				</div>
			)}
							...
							<div className="flex items-center gap-3">
								...
								<span
									className={cn(
										...
										aiHealth.trend === "improving"
											? "bg-emerald-500/15 text-emerald-400"
											: aiHealth.trend === "declining"
								...
								<div className="flex-1">
									...
									<div className="h-1.5 overflow-hidden rounded-full bg-white/10">
										<div
											...
											style={{ width: `${aiHealth.churn.risk}%` }}
										/>
									</div>
								</div>
							</div>
							...
							{aiHealth.flags.length > 0 && (
								<div className="flex flex-wrap gap-1.5">
									{aiHealth.flags.map((f) => (
										<span
											key={f}
											className="rounded-full bg-rose-500/10 px-2 py-0.5 text-[10px] text-rose-300"
										>
											⚠ {f}
										</span>
									))}
			...
			{pinned.length > 0 && !showSearch && (
				<div className="flex items-start gap-2 border-b border-line bg-gold/[0.06] px-3 py-2">
					...
					<button
						onClick={() =>
							act.mutate({ id: pinned[pinned.length - 1].id, action: "unpin" })
						}
									...
									{isEditing ? (
										...
										>
											...
											{m.toxicity_score != null &&
												m.toxicity_score < 0.7 &&
												m.toxicity_score > 0.5 && (
											...
											<div
												className={cn(
													"mt-0.5 text-[10px]",
													isMe ? "text-ink/60" : "text-muted/60",
												)}
											>
											...
											>
												...
												{isMe && (
													<>
														...
														<button
															onClick={() =>
																act.mutate({ id: m.id, action: "recall" })
															}
															className="rounded-full p-1 hover:bg-white/10"
														>
												...
												>
													<Pin
														className={cn(
															"h-3 w-3",
															m.is_pinned ? "text-gold" : "text-muted",
														)}
													/>
												</button>
											</div>
			...
			<div className="flex items-center gap-2 border-t border-line pt-3">
				...
				<button
					onClick={() => {
						send.mutate({ content: "🎤 Voice message" });
					}}
			...
			{locationSheet === "share" && (
				<ShareLocationSheet
					onShare={(lat, lng) => {
						send.mutate({
							content: JSON.stringify({ type: "Location", lat, lon: lng }),
						});
