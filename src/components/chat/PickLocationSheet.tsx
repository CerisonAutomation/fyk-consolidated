...
export function PickLocationSheet({
	onShare,
	onClose,
}: PickLocationSheetProps) {
	...
	const resolveLabel = useCallback(async (lat: number, lng: number) => {
		...
		try {
			...
			if (result) {
				const parts = [result.address, result.city, result.country].filter(
					Boolean,
				);
	...
	return (
		<div className="fixed inset-0 z-50 flex flex-col bg-black/80">
			{/* Search bar */}
			<div
				className="relative z-10 px-4 pt-4 pb-2 bg-surface/95 backdrop-blur-xl border-b border-line/50"
			...
			<div className="flex-1 relative">
				<MapPicker onPick={handlePick} onCancel={onClose} />
			...
			{/* Bottom bar */}
			<div
				className="flex items-center justify-between px-4 py-4 bg-surface border-t border-line"
				style={{
