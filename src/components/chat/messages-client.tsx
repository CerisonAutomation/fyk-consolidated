...
import { MessageCircle, Sparkles, Zap, Users, Search } from "lucide-react";
...
export function MessagesClient() {
  ...
  async function loadDigest() {
    ...
    try {
      ...
      if (withMsgs.length === 0) { setDigest({ bullets: ["No conversations to summarise"], actionItems: [] }); return; }
      ...
      setDigest({ bullets: ["AI digest unavailable right now"], actionItems: [] });
    } finally {
  ...
  if (activeId) {
    return <ChatView conversationId={activeId} onBack={() => setActiveId(null)} />;
  }
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-sm text-muted">
        Your conversations, ranked by recency. AI helps you never lose a good one.
      </p>
      ...
      <div className="mb-4 rounded-2xl border border-gold/20 bg-gold/[0.05] p-3">
        <div className="mb-2 flex items-center gap-2">
          ...
          <h2 className="text-xs font-semibold text-gold-soft">AI conversation digest</h2>
          <button
            onClick={loadDigest}
            ...
            {digest.actionItems.length > 0 && (
              <div className="mt-2 space-y-1 border-t border-gold/15 pt-2">
                {digest.actionItems.map((a, i) => (
                  <p key={i} className="flex items-start gap-1.5 text-xs font-medium text-gold-soft">
      ...
      <div className="mb-4 space-y-2">
        ...
        <div className="flex gap-1.5">
          {([
            ...
            ["groups", `Groups (${conversations.filter((c) => c.type === "group").length})`],
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          action={!search.trim() ? <Button onClick={() => (window.location.href = "/discover")}>Go discover</Button> : undefined}
        />
      ) : (
        <div className="space-y-2">
          {filtered.map((c) => {
            const isGroup = c.type === "group";
            const label = isGroup ? c.name ?? "Group" : c.otherUser?.pseudo ?? "User";
            return (
              <button
                key={c.id}
                ...
                className={cn(
                  ...
                  c.unread > 0 ? "border-gold/25 bg-gold/[0.05]" : "border-line bg-surface"
                )}
              >
                <div className="relative shrink-0">
                  {isGroup ? (
                    ...
                    <Avatar name={c.otherUser?.pseudo ?? ""} photoUrl={c.otherUser?.photos?.[0]} size={48} online={c.otherUser?.online} />
                  )}
                </div>
