...
export function GuideClientOmega() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => ({
      items: Array.from({ length: 12 }, (_, i) => ({
        id: `tribes-${i}`,
        name: `tribes ${i + 1}`,
        description: "Venue guide, map, reviews, photos, hours, travel, vs Romeo travel, Grindr Explore",
        members: Math.floor(Math.random() * 100) + 10,
        verified: Math.random() > 0.5,
        boosted: i < 2,
        distance: Math.floor(Math.random() * 5000),
        tags: ["gay", "community", "guide"],
      })),
      total: 42,
      online: 12,
    }),
  ...
  return (
    <div className="mx-auto max-w-5xl p-4 pb-24">
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white/80 p-5 shadow-[0_1px_2px_rgba(0,0,0,0.04),0_4px_12px_rgba(0,0,0,0.06)] backdrop-blur-xl">
        <h1 className="font-display text-[30px] font-bold tracking-[-0.02em] text-black capitalize">guide</h1>
        <p className="mt-1 text-[14px] text-zinc-500">Venue guide, map, reviews, photos, hours, travel, vs Romeo travel, Grindr Explore</p>
      ...
      <div className="mt-8 rounded-[16px] border bg-zinc-50 p-4">
        ...
        <ul className="mt-2 grid gap-1.5 text-[12px] text-zinc-600 md:grid-cols-2">
          <li>• Venue guide with map, reviews, photos, hours</li>
          <li>• Travel mode 2 weeks prior</li>
          <li>• Map with pins, geohash</li>
          <li>• Reviews, photos, verification</li>
        </ul>
      </div>
    </div>
  );
