...
import type { AlbumItem, Candidate } from "@/lib/types";
...
export function UserProfileClient({ userId }: { userId: string }) {
	...
	const { data: albums } = useQuery({
		...
		queryFn: () =>
			api<{ albums: AlbumItem[] }>(
				`/api/social?view=albums&userId=${userId}`,
			).then((r) => r.albums),
	});
	...
	const trackView = useMutation({
		mutationFn: () =>
			api("/api/discover", { method: "POST", body: { targetId: userId } }),
	});
	useEffect(() => {
		if (!isMe)
			trackView.mutate(); /* eslint-disable-next-line react-hooks/exhaustive-deps */
	}, [userId]);
	...
	const tap = useMutation({
		...
		onSuccess: (r) => {
			pushToast(
				r.isMatch ? "It's a match! ✨" : "Tap sent 👑",
				r.isMatch ? "info" : "success",
			);
		},
	});
	...
	const fav = useMutation({
		mutationFn: () =>
			api<{ favorited: boolean }>("/api/social", {
				method: "POST",
				body: { action: "favorite", targetId: userId },
			}),
		onSuccess: (r) =>
			pushToast(
				r.favorited ? "Added to favorites ❤️" : "Removed",
				r.favorited ? "success" : "info",
			),
	});
	...
	const saveNote = useMutation({
		mutationFn: () =>
			api("/api/notes", {
				method: "POST",
				body: { targetId: userId, content: note },
			}),
		onSuccess: () => {
		...
		return (
			<div className="mx-auto max-w-2xl">
				...
				<EmptyState
					icon="👤"
					title="This is you!"
					description="Edit your profile to see changes here."
				/>
			</div>
		);
	}
	...
	return (
		<div className="mx-auto max-w-2xl">
			...
			<div className="overflow-hidden rounded-3xl border border-line bg-surface">
				<div className="relative h-64 sm:h-80">
					...
					<div className="absolute right-4 top-4 rounded-2xl bg-ink/70 px-3 py-2 text-center backdrop-blur">
						<div className="text-2xl font-bold leading-none text-gradient-gold">
							{u.matchScore}%
						</div>
				...
				<div className="p-5">
					...
					<div className="mt-5 flex flex-wrap gap-2">
						...
						>
							<Heart
								className={cn("h-4 w-4", u.isFavorite && "fill-rose-400")}
							/>
						</button>
						<button
			...
			<div className="mt-4 grid grid-cols-2 gap-2">
				{u.bodyType && (
					<Detail icon={<Weight className="h-4 w-4" />} label={u.bodyType} />
				)}
				{u.occupation && (
					<Detail
				...
				{u.languages.length > 0 && (
					<Detail
						...
						label={u.languages.join(", ")}
					/>
				)}
				<Detail
					icon={<Crown className="h-4 w-4" />}
					label={`Trust ${u.trustScore}/100`}
				/>
			</div>

			{u.tribes.length > 0 && (
				<Tags title="Tribes" items={u.tribes} color="purple" />
			)}
			{u.interests.length > 0 && <Tags title="Interests" items={u.interests} />}
			{u.lookingFor.length > 0 && (
				<Tags title="Looking for" items={u.lookingFor} gold />
			)}
			{u.position.length > 0 && (
				<Tags title="Position" items={u.position} color="blue" />
			)}
			...
			<div className="mt-4 rounded-2xl border border-line bg-surface p-4">
					...
					<button
						onClick={async () => {
							const r = await api<{ icebreakers: string[] }>("/api/ai", {
								method: "POST",
								body: { action: "icebreakers", targetId: u.id },
							});
				...
				{icebreakers ? (
					...
					<p className="text-xs text-muted">
						Generate personalised openers based on {u.pseudo}&apos;s profile.
					</p>
				)}
			</div>
