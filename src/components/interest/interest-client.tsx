...
import { Crown, Eye, FileText, Heart, Star, Users } from "lucide-react";
...
export function InterestClient() {
	...
	const { data: stats, isLoading: statsLoading } = useQuery({
		...
		queryFn: () =>
			api<{ stats: Stats }>("/api/interest/stats").then((r) => r.stats),
	});

	const { data: profiles, isLoading: profilesLoading } = useQuery({
		queryKey: ["interest", activeTab],
		queryFn: () =>
			api<{ profiles: InterestProfile[] }>(`/api/interest/${activeTab}`).then(
				(r) => r.profiles,
			),
	});
					...
					<div className="flex items-center gap-3">
						...
						<div className="flex-1">
							<p className="text-sm font-semibold text-white">
								Free shows the three most recent likes clearly. Plus reveals
								everyone.
							</p>
						</div>
