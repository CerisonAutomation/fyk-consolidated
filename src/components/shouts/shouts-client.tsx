...
import { listShouts, createShout, toggleShoutLike, type ShoutItem, type ShoutAuthor } from "#/integrations/supabase/shouts";
...
function authorDisplay(a: ShoutAuthor | null) {
  ...
  return {
    ...
    online: !a.hide_online && Date.now() - new Date(a.last_active_at).getTime() < 5 * 60_000,
    tribes: [] as string[],
    ...
    onMutate: async (shoutId) => {
      ...
      if (prev) {
        qc.setQueryData(
          ...
          prev.map((s) =>
            s.id === shoutId
              ? { ...s, has_liked: !s.has_liked, likes_count: s.likes_count + (s.has_liked ? -1 : 1) }
              : s,
          ),
        );
      }
          ...
          {shouts.map((s) => {
            ...
            return (
              <div key={s.id} className="rounded-2xl border border-line bg-surface p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-1.5">
                      <p className="text-sm font-semibold text-white">{author.pseudo}</p>
                      {author.verified && <Sparkles className="h-3 w-3 text-gold" />}
                    </div>
                ...
                >
                  <Heart className={cn("h-4 w-4", s.has_liked && "fill-rose-400")} />
                  {s.likes_count}
                </button>
              </div>
            );
