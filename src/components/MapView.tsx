...
export function MapView({
  ...
  useEffect(() => {
    ...
    for (const p of pins) {
      ...
      const marker = L.marker([p.lat, p.lng], { icon, title: p.label, keyboard: true }).addTo(layer);
      marker.bindTooltip(`${p.label}${p.sub ? ` · ${p.sub}` : ""}`, { direction: "top", offset: [0, -18] });
  ...
  if (failed) {
    return (
      <div
        className={cn("overflow-hidden rounded-2xl border border-line bg-surface-2 p-5", className)}
        style={{ minHeight: height }}
      >
        <div className="flex items-center gap-2.5 text-muted">
          <WifiOff className="h-[18px] w-[18px]" />
          <p className="text-[13.5px] font-medium">Map tiles unavailable — showing the list instead.</p>
        </div>
            ...
            <li key={p.id}>
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-[13.5px] font-medium text-ink">{p.label}</span>
                  {p.sub && <span className="block truncate text-[12px] text-muted">{p.sub}</span>}
                </span>
              </button>
            </li>
          ))}
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-2xl border border-line", className)}>
      ...
      {!ready && <div className="skeleton pointer-events-none absolute inset-0" />}
...
export function pinFrom(coords: LatLng, extra: Omit<MapPinItem, "lat" | "lng">): MapPinItem {
