...
import { COMMANDS, speak, VoiceController } from "@/lib/voice";
...
export function CommandPalette() {
	...
	const actions = useMemo<Action[]>(() => {
		...
		base.push(
			...
			{
				id: "f-online",
				label: "Toggle Online-now filter",
				icon: Sparkles,
				group: "Discover",
				run: () => toggleFilter("online"),
			},
			{
				id: "f-clear",
			...
			},
			{
				...
				icon: LayoutGrid,
				group: "Discover",
				run: () => setLayout("cascade"),
			},
			{
				id: "map",
				label: mapOpen ? "Hide the map" : "Show the map",
				icon: MapPin,
				group: "Discover",
			...
			{
				id: "ai",
				label: "Load the on-device AI model",
				hint: "all-MiniLM-L6-v2 · int8",
				icon: Wand2,
				group: "AI",
				run: warmUpAi,
			},
			{
				...
				run: () =>
					toast(
						"Set an app-lock PIN first: Settings \u2039 Security.",
						"violet",
					),
			},
			{
				...
				run: () =>
					speak(
						`You are on the ${store.view} screen. ${richPeople.length} sample profiles are loaded.`,
					),
			},
		);
	...
	const results = useMemo(() => {
		...
		return actions
			.filter((a) =>
				`${a.label} ${a.hint ?? ""} ${a.group}`.toLowerCase().includes(needle),
			)
			.slice(0, 14);
	}, [q, actions]);
		...
		<div className="fixed inset-0 z-[110] flex items-start justify-center px-4 pt-[12vh]">
			...
			>
				<div className="flex items-center gap-3 border-b border-line px-4">
					...
					<input
						...
						aria-activedescendant={
							results[cursor] ? `cmd-${results[cursor].id}` : undefined
						}
				...
				<ul
					...
					role="listbox"
					aria-label="Commands"
					className="max-h-[52vh] overflow-y-auto scroll-thin p-2"
				>
					{results.length === 0 && (
						<li className="px-3 py-6 text-center text-[13.5px] text-muted">
							No matches.
						</li>
					)}
					{results.map((a, i) => (
						<li key={a.id}>
							...
							>
								<a.icon
									className="h-[17px] w-[17px] shrink-0 text-faint"
									strokeWidth={1.8}
								/>
		...
		c.on((id) => {
			switch (id) {
				...
				case "go:admin":
					return go("admin");
				case "filter:online":
					return toggleFilter("online");
				case "filter:verified":
					return toggleFilter("verified");
				case "filter:clear":
					return clearFilters();
				case "layout:compact":
					return setLayout("compact");
				case "layout:cascade":
					return setLayout("cascade");
				case "map:toggle":
					return setMapOpen(!mapOpen);
				case "palette:open":
				...
				case "chat:read": {
					...
					const last = t?.messages
						.slice(-3)
						.map(
							(m: { from?: string; body?: string }) =>
								`${m.from === "me" ? "You said" : "They said"} ${m.body ?? ""}`,
						);
	...
	useEffect(() => {
		...
		if (voiceOn) {
			...
			if (!ok && !c.supported) {
				setVoiceOn(false);
				toast(
					"Voice control isn't supported here: Try Chrome, Edge or Safari.",
					"violet",
				);
			}
		} else {
		...
		<div className="pointer-events-none fixed inset-x-0 bottom-[86px] z-[100] flex justify-center px-4 lg:bottom-6">
			<div className="anim-sheet pointer-events-auto flex max-w-[min(94vw,32rem)] items-center gap-3 rounded-full border border-gold/40 bg-surface/95 py-2.5 pl-4 pr-2.5 shadow-[var(--shadow-pop)] backdrop-blur">
				...
				<p className="min-w-0 flex-1 truncate text-[13.5px] text-ink-2">
					{voiceTranscript ||
						'Listening — try "go to chats" or "message theo see you at eight".'}
				</p>
				<button
					type="button"
