  ...
  return (
    ...
    >
      {toasts.map((t) => (
        ...
        >
          {t.type === "success" && <CheckCircle2 className="h-4 w-4 text-emerald-400" aria-hidden="true" />}
          {t.type === "error" && <AlertCircle className="h-4 w-4 text-rose-400" aria-hidden="true" />}
          {t.type === "info" && <Info className="h-4 w-4 text-gold" aria-hidden="true" />}
          <span>{t.message}</span>
          <button
            type="button"
