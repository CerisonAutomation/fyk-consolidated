...
async function inspectBackend(userId?: string): Promise<BackendReadiness> {
	...
	const [
		...
		client
			.from("profiles")
			.select(
				"id,handle,age_verified_at,exposure_level,onboarding_completed_at",
			)
			.limit(1),
		...
		client
			.from("messages")
			.select("id,conversation_id,album_share_id")
			.limit(1),
		client
			.from("message_attachments")
			.select("id,message_id,access_policy,status")
			.limit(1),
		client
			.from("album_shares")
			.select("id,album_id,access_policy,status")
			.limit(1),
		client
		...
		client
			.from("private_albums")
			.select("id,owner_id,default_access_policy")
			.limit(1),
		client
	...
	if (privateProfile.error)
		missing.push("profile_private table for date of birth");
	...
	if (matches.error)
		missing.push("matches table or mutual-like trigger policies");
	if (conversations.error) missing.push("conversations table or member policy");
	if (messages.error)
		missing.push("messages.album_share_id from 004_chat_media.sql");
	if (attachments.error)
		missing.push("message_attachments from 004_chat_media.sql");
	if (shares.error) missing.push("album_shares from 004_chat_media.sql");
	if (reactions.error)
		missing.push("message_reactions from 004_chat_media.sql");
	if (albums.error) missing.push("private_albums from 004_chat_media.sql");
	if (albumItems.error)
		missing.push("private_album_items album columns from 004_chat_media.sql");
	...
	if (userId) {
		...
		if (avatarBucket.error)
			missing.push("avatars-public bucket or authenticated storage policy");
		if (chatBucket.error)
			missing.push("chat-media-private bucket or fyk_chat_* policies");
		if (albumBucket.error)
			missing.push("albums-private bucket or fyk_album_* policies");
	}
	...
	const load = useCallback(async () => {
		...
		if (!publicReadiness.coreProfile) {
			setGate({
				kind: "setup",
				reason:
					"The Supabase project is connected, but the profile schema required for auth and onboarding is incomplete.",
		...
		if (error) {
			setGate({
				kind: "setup",
				reason: "Session restore failed. Check the Supabase URL and anon key.",
				missing: ["Valid auth session"],
			});
			return;
		}
		if (!data.session) {
		...
		if (profileResult.error) {
			setGate({
				kind: "setup",
				reason: friendlySchemaError(profileResult.error.message),
				missing: ["Readable authenticated profile"],
			});
			return;
		}
		...
		<div className="relative min-h-[100svh] overflow-hidden bg-canvas text-ink">
			...
			<main className="relative mx-auto flex min-h-[100svh] max-w-5xl items-center px-5 py-12 sm:px-8">
				<div className="max-w-2xl">
					...
					<p className="mt-8 text-[11px] font-bold uppercase tracking-[0.24em] text-gold">
						Setup required
					</p>
					...
					<p className="mt-5 max-w-xl text-[12.5px] leading-relaxed text-faint">
						FYKING will not fall back to a demo account while Supabase is
						configured. This prevents sample profiles, local paywalls and
						browser-only privacy controls from being mistaken for real user
						data.
					</p>
				</div>
			</main>
...
function SignedOut() {
	...
	const submitSignIn = async (data: z.infer<typeof authSchema>) => {
		...
		try {
			...
			if (authError)
				setError(
					"We couldn't sign you in. Check your details or confirm your email first.",
				);
		} finally {
			setBusy(false);
		}
			...
			<main className="relative mx-auto grid min-h-[100svh] max-w-7xl items-center gap-10 px-5 py-10 lg:grid-cols-[minmax(0,1fr)_410px] lg:px-10">
				<section className="max-w-2xl pt-4 lg:pt-0">
					...
					<p className="mt-5 max-w-xl text-[16px] leading-relaxed text-white/68 sm:text-[18px]">
						Nearby, chat, private albums, events and spontaneous plans. One
						adults-only place for a date, a beach day, a coffee or something
						more direct.
					</p>
					<div className="mt-8 flex flex-wrap gap-x-7 gap-y-3 text-[13px] font-medium text-white/62">
						<span className="inline-flex items-center gap-2">
							<ShieldCheck className="h-4 w-4 text-gold" /> 18+ only
						</span>
						<span className="inline-flex items-center gap-2">
							<MapPin className="h-4 w-4 text-gold" /> Approximate location by
							default
						</span>
						<span className="inline-flex items-center gap-2">
							<LockKeyhole className="h-4 w-4 text-gold" /> Supabase session
						</span>
					</div>
				</section>
				...
				<section className="rounded-[24px] border border-white/12 bg-black/48 p-5 shadow-2xl backdrop-blur-xl sm:p-7">
					...
					{mode === "signin" && (
						...
						>
							<label className="block">
								<span className="mb-1.5 block text-[12.5px] font-semibold text-white/72">
									Email
								</span>
								...
								{signinForm.formState.errors.email && (
									<p className="mt-1 text-[12px] text-live" role="alert">
							...
							{error && (
								<p
									role="alert"
									className="rounded-xl border border-live/35 bg-live/12 px-3.5 py-3 text-[13px] text-[#ff9aa6]"
								>
									{error}
								</p>
							)}
							{notice && (
								<p
									role="status"
									className="rounded-xl border border-online/35 bg-online/10 px-3.5 py-3 text-[13px] text-[#8ee2b4]"
								>
									{notice}
								</p>
							)}
					...
					{mode === "forgot" && (
						<form
							onSubmit={forgotForm.handleSubmit(submitForgot)}
							className="mt-6 space-y-4"
							noValidate
						>
							<label className="block">
	...
	return (
		<div className="grid min-h-[100svh] place-items-center bg-canvas px-5 py-10 text-ink">
			<main className="w-full max-w-md rounded-[24px] border border-line bg-surface p-6 shadow-[var(--shadow-pop)] sm:p-8">
				...
				<h1 className="mt-4 text-[29px] font-bold tracking-[-0.03em]">
					Choose a new password
				</h1>
				<p className="mt-2 text-[14px] leading-relaxed text-muted">
					This recovery session came from your Supabase email link. After
					updating, you will sign in again with the new password.
				</p>
				<form
					...
					<Field label="New password" error={errors.password?.message}>
						<div className="relative">
							<input
								...
								className={cn(
									"entry-input pr-11",
									errors.password && "border-live/50",
								)}
...
function Onboarding({
	...
	const next = () => {
		...
		if (displayName.trim().length < 2)
			return setError("Use at least 2 characters for your display name.");
		if (!/^[a-z0-9_]{3,24}$/.test(handle.trim().toLowerCase()))
			return setError("Handle must be 3-24 letters, numbers or underscores.");
		if (!dob || dob > maxDob || age < 18)
			return setError("FYKING is for adults aged 18 and over.");
	...
	const finish = async () => {
		if (!confirmed) {
			setError(
				"Confirm the adult-content and privacy defaults before continuing.",
			);
			return;
		}
		const client = getSupabase();
						...
						{step === 1 ? (
							<div className="space-y-4">
								...
								<p className="text-[12px] leading-relaxed text-faint">
									Your birth date is used for the 18+ gate. Other members see
									only your derived age.
								</p>
								{error && (
									<p role="alert" className="text-[13px] text-live">
										{error}
							...
							<div>
								<div className="space-y-3">
									{[
										[
											ShieldCheck,
											"Content starts Clean",
											"Mature tags and media stay hidden until you opt in.",
										],
										[
											MapPin,
											"Distance stays approximate",
											"The app stores only a deliberately coarsened location.",
										],
										[
											LockKeyhole,
											"Your controls are yours",
											"Block, report and album access are designed for server policies.",
										],
									].map(([Icon, title, body]) => {
								...
								<label className="mt-5 flex cursor-pointer items-start gap-3 rounded-xl border border-line bg-surface-2 p-3.5">
									...
									<span className="text-[12.5px] leading-relaxed text-ink-2">
										I confirm I am 18+ and understand that FYKING includes
										optional adult-oriented discovery and media controls.
									</span>
								</label>
								{error && (
								...
								<div className="mt-5 flex gap-2">
									...
									<button
										...
										onClick={() => void finish()}
										disabled={busy}
										className="press flex h-12 flex-1 items-center justify-center gap-2 rounded-full bg-gold text-[14.5px] font-bold text-black hover:bg-gold-2 disabled:opacity-55"
									>
										{busy ? (
											<Loader2 className="h-4 w-4 animate-spin" />
										) : (
