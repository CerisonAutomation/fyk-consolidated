...
function CreateFansiteForm({ onDone }: { onDone: () => void }) {
	...
	const createMutation = useMutation({
		mutationFn: () =>
			createFansite(user?.id, { name, description: description || undefined }),
		onSuccess: (result) => {
			if (result.ok) {
...
export function FansitesClient() {
	...
	const { data, isLoading } = useQuery({
		...
		queryFn: () =>
			user
				? listFansites(user.id).then((r) => (r.ok ? r.data : []))
				: Promise.resolve([]),
		enabled: !!user,
	});
	...
	return (
		<div className="mx-auto max-w-2xl">
			...
			<p className="mb-5 text-sm text-muted">
				Subscribe to the kings whose content you love. Photoshoots, training
				logs, travel diaries.
			</p>
			...
			{isLoading ? (
				...
				<div className="space-y-3">
					{fansites.map((f) => {
						...
						const firstPhoto =
							Array.isArray(ownerPhotos) && ownerPhotos.length > 0
								? ownerPhotos[0]
								: undefined;
