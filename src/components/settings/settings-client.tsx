...
export function SettingsClient() {
	...
	useEffect(() => {
		...
		async function load() {
			...
			if (data.colorblind_mode)
				setColorblind(
					data.colorblind_mode === true
						? "off"
						: (data.colorblind_mode ?? "off"),
				);
	...
	const flip = useCallback(
		async (key: keyof typeof prefs) => {
			...
			pushToast(
				`${key.replace(/([A-Z])/g, " $1").replace(/^./, (c) => c.toUpperCase())} ${next ? "on" : "off"}`,
				"info",
			);
	...
	const handleAccent = useCallback(
		(id: string) => {
			...
			pushToast(
				`Accent: ${ACCENTS.find((a) => a.id === id)?.label ?? id}`,
				"info",
			);
		},
	...
	const handleFontSize = useCallback(
		(id: string) => {
			...
			pushToast(
				`Font: ${FONT_SIZES.find((f) => f.id === id)?.label ?? id}`,
				"info",
			);
		},
		[persistUpdate, pushToast],
	...
	const handleDeleteAccount = useCallback(async () => {
		...
		try {
			...
			// Delete the profile row. Child rows (taps, stories, shouts, …) are
			...
			pushToast(
				`Deletion failed: ${(err as Error).message}. Contact support for help.`,
				"error",
			);
		} finally {
			setDeleting(false);
			...
			items: [
				{
					...
					desc: "Intent-based replies in chat",
				},
				{
					key: "aiTranslation",
					label: "Auto-translation",
					desc: "Translate messages to your language",
				},
				{
					key: "aiModeration",
					label: "Safety moderation",
					desc: "Auto-flag harassment, NSFW and scams",
				},
				{
					key: "aiMemory",
					label: "Conversation memory",
					desc: "AI remembers context across chats",
				},
				{
					key: "autoReply",
					label: "Auto-reply",
					desc: "AI replies when you're away (Gold)",
				},
			],
		},
		{
			...
			items: [
				...
				{ key: "eventNotifications", label: "Event reminders" },
				{
					...
					label: "Smart timing",
					desc: "Deliver at moments you actually reply",
				},
		...
		},
		{
			...
			items: [
				...
				{ key: "readReceipts", label: "Read receipts" },
				{
					...
					desc: "Browse without being seen (Gold)",
				},
				{
					key: "discreetMode",
					label: "Discreet app icon",
					desc: "Neutral icon and name on home screen",
				},
			],
		},
		{
			...
			items: [
				{
					key: "voiceCommands",
					label: "Voice control",
					desc: "Navigate and compose by voice",
				},
			...
			items: [
				{
					key: "offlineMode",
					label: "Offline mode",
					desc: "Queue actions when connection drops",
				},
			],
		},
	];
			...
			<div className="mb-4 flex items-center gap-3 rounded-2xl border border-gold/30 bg-gradient-to-br from-gold/12 to-transparent p-4">
				...
				<div className="flex-1">
					<h2 className="font-semibold capitalize text-white">
						{me?.tier ?? "free"} membership
					</h2>
					<p className="text-[11px] text-muted">
						{me?.tier === "free"
							? "Upgrade for unlimited taps, incognito and all 48 AI features."
							: "Thanks for supporting FYK"}
					</p>
				</div>
				<Button size="sm" onClick={() => navigate({ to: "/premium" })}>
										...
										if (item.custom === "dnd") {
											return (
												<div key={item.label} className="py-3">
													...
													<div className="flex gap-1.5">
														{(["off", "scheduled", "always"] as const).map(
															(m) => (
																<button
																	key={m}
																	onClick={() => handleDnd(m)}
