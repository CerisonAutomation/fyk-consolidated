...
export function CheckInBar() {
	...
	useEffect(() => {
		if (checkIn && checkIn.status === "ARMED" && now > checkIn.dueAt)
			...
			<div
				className={cn(
					...
					overdue
						...
						: "border-gold/45 bg-surface/95",
				)}
			>
				<span
					className={cn(
						"grid h-10 w-10 shrink-0 place-items-center rounded-xl",
						overdue ? "bg-live/20 text-live" : "bg-gold-ghost text-gold",
					)}
				>
					{overdue ? (
						<AlertTriangle className="h-[19px] w-[19px]" aria-hidden="true" />
					) : (
...
function CodeGrid({ seed, size = 21 }: { seed: string; size?: number }) {
	...
	return (
		<svg
			...
			role="img"
			aria-label="Your rotating profile code"
		>
	...
	return (
		<Modal open onClose={() => setQrOpen(false)} labelledBy="party-title">
			<div className="p-6 text-center">
				...
				<h2
					id="party-title"
					className="mt-4 text-[21px] font-bold tracking-[-0.01em] text-ink"
				>
					Party mode
				</h2>
				<p className="mx-auto mt-2 max-w-sm text-[14px] leading-relaxed text-muted">
					Swap profiles in person without handing over a phone number. The code
					rotates every ten minutes and nothing about it touches a server.
				</p>
				...
				<p className="mt-3 text-[12.5px] font-medium text-gold">
					Rotates in {Math.floor(seconds / 60)}:
		...
		<div className="anim-pop rounded-xl border border-line bg-surface-2 p-3.5">
			<div className="mb-2.5 flex items-center gap-2">
				...
				<button
					type="button"
					onClick={onClose}
					aria-label="Close footprint picker"
					className="press ml-auto text-faint hover:text-ink"
				>
					...
					<button
						...
						className={cn(
							...
							current === f.id
								...
								: "border-line bg-surface text-ink-2 hover:border-violet/50 hover:text-violet",
						)}
					>
			...
			<p className="mt-2.5 text-[11.5px] text-faint">
				Compliments only — FYK has no body ranking or scoring.
			</p>
		</div>
	);
}
