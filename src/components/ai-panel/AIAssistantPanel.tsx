...
import { usePhotoScores } from "#/hooks/app-hooks";

export function PhotoEnhancerPanel() {
  const { scorePhoto } = usePhotoScores();
  ...
  return (
    <div className="p-4 border rounded-xl space-y-3">
      ...
      <p className="text-xs text-muted-foreground">Scores quality and appeal. Blocks identity changes to prevent catfishing.</p>
      <div className="flex gap-2">
        <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="Photo URL" className="flex-1 border rounded p-2 text-sm" />
        <button onClick={handleScore} className="px-3 py-1 bg-primary text-white rounded text-sm">
          Score
        </button>
      </div>
      {scores.map((s, i) => (
        <div key={i} className="text-xs border rounded p-2">
          <div>
            Quality: {s.quality} | Lighting: {s.lighting} | Blur: {s.blur} | Smile: {s.smile} | Appeal: {s.appeal}
          </div>
...
export function TranslationPanel() {
  ...
  return (
    <div className="p-4 border rounded-xl space-y-3">
      ...
      <p className="text-xs text-muted-foreground">Inline per bubble, auto source detection, see original toggle</p>
      ...
      <div className="flex gap-2">
        ...
        <button onClick={translate} disabled={loading} className="px-3 py-1 bg-primary text-white rounded text-sm">
          {loading ? "..." : "Translate"}
        </button>
      </div>
      {result && (
...
export function AutocompleteDemo() {
  ...
  return (
    <div className="p-4 border rounded-xl space-y-2">
      ...
      <p className="text-xs text-muted-foreground">Predicts rest of sentence in your voice, tap to accept</p>
      <div className="relative">
        <input
          value={prefix}
          onChange={(e) => check(e.target.value)}
          placeholder="Type hey, how are, what are..."
        ...
        {completion && (
          <div className="absolute top-full left-0 right-0 bg-white border rounded mt-1 p-2 text-sm shadow-lg">
            ...
            <div className="text-xs text-muted-foreground">
              Source: {completion.source} ({Math.round(completion.confidence * 100)}%)
            </div>
          </div>
        )}
      </div>
...
export function MemeSuggestPanel() {
  ...
  return (
    <div className="p-4 border rounded-xl space-y-2">
      ...
      <p className="text-xs text-muted-foreground">Filters semantically from current message</p>
      <div className="flex gap-2">
        ...
        <button onClick={suggest} className="px-3 py-1 bg-primary text-white rounded text-sm">
          Suggest
        </button>
      </div>
...
export function VoiceNotePanel() {
  ...
  return (
    <div className="p-4 border rounded-xl space-y-2">
      ...
      <p className="text-xs text-muted-foreground">Generate warm voice note in your tone</p>
      ...
      <div className="flex gap-2">
        ...
        <button onClick={generate} className="px-3 py-1 bg-primary text-white rounded text-sm">
          Generate
        </button>
      </div>
      {voiceNote && (
        <div className="text-xs p-2 bg-muted rounded">
          Duration: {voiceNote.durationSec}s | Tone: {voiceNote.tone} | Transcript: {voiceNote.transcript}
        </div>
      )}
    </div>
  );
