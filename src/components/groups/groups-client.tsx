...
export function GroupsClient() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const payload = await api<Record<string, unknown>>(`/api/groups?${params.toString()}`);
      return {
        items: listOf<GroupsItem>(payload, "items"),
  ...
  const boostMutation = useEntityAction("group", "boost", {
    invalidate: ["groups"],
    onDone: () => vibrate(20),
    onRefused: setActionNote,
  });
  const engageMutation = useEntityAction("group", "engage", {
    invalidate: ["groups"],
    onDone: () => vibrate(10),
    onRefused: setActionNote,
  });
