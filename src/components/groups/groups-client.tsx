...
import { Users, Check, Plus, MessageCircle, Send, ArrowLeft } from "lucide-react";
...
function GroupMessages({
  ...
  const { data: messages, isLoading } = useQuery({
    ...
    queryFn: () =>
      user ? loadGroupMessages(group.id, user.id).then((r) => (r.ok ? r.data : [])) : Promise.resolve([]),
    ...
    <div className="flex flex-col">
      <div className="mb-3 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="truncate text-sm font-bold text-white">{group.name}</h2>
      ...
      <div className="mb-3 max-h-[50vh] space-y-2 overflow-y-auto rounded-2xl border border-line bg-surface p-3">
        {isLoading ? (
          ...
          <p className="py-6 text-center text-xs text-muted">No messages yet. Start the conversation!</p>
        ) : (
          [...messages].reverse().map((m) => (
            ...
            >
              ...
              >
                {m.sender_id !== user?.id && (
                  <p className="mb-0.5 text-[10px] font-semibold text-gold/70">{m.sender_name}</p>
                )}
                <p>{m.content}</p>
              </div>
            </div>
...
function CreateGroupForm({ onDone }: { onDone: () => void }) {
  ...
  const createMutation = useMutation({
    mutationFn: () => createGroup(user?.id, { name, description: description || undefined, icon }),
    onSuccess: (result) => {
      if (result.ok) {
