...
export function MediaViewer() {
	const { media, mediaIndex, setMediaIndex, closeMedia, toast } = useStore();
	const { profile } = useAuth();
	const viewerName =
		profile.display_name || profile.handle || "Signed-in viewer";
	...
	const src =
		item.url ?? (item.photo ? px(Number(item.photo), 1400, 1900) : "");
	...
	return (
		<div
			...
			className="fixed inset-0 z-[135] flex flex-col bg-black"
		>
			{/* header */}
				...
				<div className="min-w-0 flex-1">
					...
					<p className="flex items-center gap-2 text-[12px] text-white/60">
						...
						<span className="inline-flex items-center gap-1">
							<Eye className="h-3 w-3" aria-hidden="true" /> Authorized in-app
							view
						</span>
					</p>
				</div>
			...
			>
				{item.kind === "video" ? (
					<video
						...
						muted={muted}
						className={cn(
							"max-h-full max-w-full object-contain transition-[filter]",
							hidden && "blur-2xl",
						)}
					/>
				) : (
			...
			<footer className="safe-b relative z-20 bg-gradient-to-t from-black/90 to-transparent px-4 pb-5 pt-10">
				{item.caption && (
					<p className="mx-auto mb-3 max-w-2xl text-center text-[14px] leading-relaxed text-white/85">
						{item.caption}
					</p>
				)}
						...
						{media.map((m, i) => (
							<button
								...
								className={cn(
									...
									i === mediaIndex ? "w-7 bg-gold" : "w-1.5 bg-white/35",
								)}
							/>
						))}
					</div>
				...
				<div className="flex items-center justify-center gap-2">
					...
					<button
						...
						onClick={() =>
							toast(
								`Saving is disabled here: ${item.ownerName} shared this inside FYK. Ask them directly if you'd like a copy.`,
								"violet",
							)
						}
					...
					<button
						...
						onClick={() =>
							toast(
								"Report captured locally: No remote moderation queue receives this yet.",
								"live",
							)
						}
				...
				<p className="mt-3 flex items-center justify-center gap-1.5 text-center text-[11px] text-white/40">
					...
					Watermarked with your name. Screenshots are deterred, not prevented —
					please respect the sender.
				</p>
			</footer>
		</div>
	);
