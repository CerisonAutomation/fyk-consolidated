...
const STAGE_EMOJI: Record<string, string> = { baby: "🐣", juvenile: "🐥", adult: "👑" };
const MOOD_EMOJI: Record<string, string> = {
  happy: "😊", hungry: "😋", sleepy: "😴", excited: "🤩", sad: "😢",
};

const ACTIONS = [
  { key: "feed", label: "Feed", icon: UtensilsCrossed, desc: "+20 XP · streak" },
...
export function KingPetClient() {
  ...
  const progress = Math.min(100, Math.round((pet.experience / (pet.level * 100)) * 100));
    ...
    <div className="mx-auto max-w-md">
      ...
      {celebrate && (
        <div className="mb-4 flex items-center gap-2 rounded-2xl border border-gold/40 bg-gold/10 p-4 animate-in">
          ...
          <span className="text-sm font-semibold text-gold-soft">{celebrate}</span>
        </div>
      )}
      ...
      <div className="relative mb-4 overflow-hidden rounded-3xl border border-line bg-surface p-6 text-center">
        ...
        <div className="relative mx-auto my-5 flex h-40 w-40 items-center justify-center rounded-full bg-gradient-to-br from-gold/20 via-purple-600/15 to-transparent">
          {equipped.has("Golden Crown") && <span className="absolute -top-1 text-3xl">👑</span>}
          {equipped.has("Sunglasses") && <span className="absolute top-12 text-2xl">🕶️</span>}
          {equipped.has("Bandana") && <span className="absolute bottom-6 text-2xl">🧣</span>}
          {equipped.has("Sneakers") && <span className="absolute -bottom-1 text-xl">👟</span>}
          {equipped.has("Cape") && <span className="absolute -left-6 top-8 text-2xl">🦸</span>}
          <span className="animate-float text-7xl">{STAGE_EMOJI[pet.stage]}</span>
        </div>
        ...
        <>
          ...
          <div className="mt-4 flex items-start gap-2 rounded-2xl border border-line bg-surface p-4">
            ...
            <p className="text-xs leading-relaxed text-muted">
              Feed <span className="text-gold-soft">{pet.name}</span> daily to build your streak.
              Streaks earn bonus bones — and bones buy adventures and outfits.
            </p>
          </div>
        </>
      )}
      ...
      {tab === "wardrobe" && (
        items.length === 0 ? (
          <EmptyState icon="👕" title="Shop is empty" description="Items will appear here." />
        ) : (
          <div className="grid grid-cols-2 gap-2.5">
            {items.map((i) => {
              ...
              const reqIdx = ["baby", "juvenile", "adult"].indexOf(i.stage_required);
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isEquipped ? "border-gold/45 bg-gold/[0.08]" : "border-line bg-surface"
                  )}
                >
                  <div className="mb-1 text-3xl">{i.emoji}</div>
                  <p className="truncate text-xs font-semibold text-white">{i.name}</p>
                  <p className="text-[10px] capitalize text-muted">{i.type} · {i.stage_required}+</p>
                  <div className="mt-2">
                    {isOwned ? (
                      <button
                        onClick={() => act.mutate({ action: "equip", name: i.name })}
                        className={cn(
                          "w-full rounded-lg py-1.5 text-[11px] font-semibold transition-colors",
                          isEquipped ? "bg-gold text-ink" : "border border-line bg-surface-2 text-white"
                        )}
                      >
                      ...
                      <button
                        onClick={() => act.mutate({ action: "buyItem", itemId: i.id })}
                ...
                <div key={a.id} className="flex items-start gap-3 rounded-2xl border border-line bg-surface p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      <p className="text-sm font-semibold text-white">{a.theme}</p>
                    ...
                    <p className="mt-1 flex items-center gap-3 text-[11px] text-muted">
                      ...
                      <span className="text-gold/70">+{a.reward_amount} {a.reward_type}</span>
                    </p>
                  </div>
                  <Button
                    size="sm"
                    ...
                    onClick={() => act.mutate({ action: "adventure", adventureId: a.id })}
                  >
                    {done ? "Again" : <><Backpack className="mr-1 h-3.5 w-3.5" />Go</>}
                  </Button>
                </div>
              );
            })}
      ...
      {tab === "log" && (
        pet.mood_log.length === 0 ? (
          <EmptyState icon="📊" title="No mood history yet" description="Interact with your pet to start tracking." />
        ) : (
          <div className="rounded-2xl border border-line bg-surface p-4">
            <div className="mb-4 flex items-end justify-between gap-1" style={{ height: 80 }}>
              {[...pet.mood_log].reverse().map((m, i) => {
                const score: Record<string, number> = { happy: 60, excited: 90, sleepy: 30, hungry: 40, sad: 20 };
                return (
      ...
      {renaming && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/85 p-4" onClick={() => setRenaming(false)}>
          <div className="w-full max-w-xs rounded-2xl border border-line bg-surface p-5" onClick={(e) => e.stopPropagation()}>
            <h3 className="mb-3 text-sm font-semibold text-white">Rename your pet</h3>
            <input
              value={name}
