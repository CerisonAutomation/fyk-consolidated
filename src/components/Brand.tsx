...
function Crown({ stroke = GOLD, width = 26 }: { stroke?: string; width?: number }) {
  return (
    <path
...
export function LogoHorizontal({ className, showRule = true }: { className?: string; showRule?: boolean }) {
  return (
    <svg viewBox="0 0 660 150" className={className} role="img" aria-label="FY KING">
      <title>FY KING</title>
      <g fontFamily="Inter, system-ui, sans-serif" fontWeight="900" fontSize="118" letterSpacing="-2">
        {/* F — solid white */}
      ...
      {showRule && <rect x="0" y="138" width="644" height="8" rx="4" fill="#FFFFFF" />}
    </svg>
  );
}
...
export function LogoStacked({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 420 300" className={className} role="img" aria-label="FY KING">
      <title>FY KING</title>
      <g fontFamily="Inter, system-ui, sans-serif" fontWeight="900" fontSize="132" letterSpacing="-3">
...
export function CrownMark({ className, tone = GOLD }: { className?: string; tone?: string }) {
  return (
    <svg viewBox="0 0 92 88" className={className} role="img" aria-label="FY KING">
      <title>FY KING</title>
  ...
  return (
    <span className={cn("inline-flex items-center gap-1.5", className)}>
      <span className="text-[17px] font-black leading-none tracking-tight text-ink">
        F<span className="text-transparent [-webkit-text-stroke:1.1px_var(--c-ink)]">Y</span>
      </span>
      <span className="text-[17px] font-black leading-none tracking-tight" style={{ color: RED }}>
