...
const MOBILE_NAV = NAV.filter((n) =>
	["nearby", "board", "chats", "events", "profile"].includes(n.id),
);
...
function NavList({ vertical = true }: { vertical?: boolean }) {
	...
	const badge = useCallback(
		(id: ViewId) =>
			id === "chats" ? unread : id === "likes" ? likesReceived : 0,
		[unread, likesReceived],
	);

	return (
		<nav
			aria-label="Primary"
			className={cn(vertical ? "flex flex-col gap-1 px-2" : "flex")}
		>
...
function AccountPanel() {
	...
	const name =
		profile.display_name || profile.handle || user.email || "Your account";
	...
	return (
		<div className="m-3 rounded-2xl border border-line bg-surface p-3">
			...
			<p className="mt-2 border-t border-line-soft pt-2 text-[11px] font-medium text-online">
...
function SecondaryNav() {
	...
	return (
		<div className="border-t border-line-soft px-2 pt-3">
			<p className="mb-1 px-4 text-[10px] font-bold uppercase tracking-[0.18em] text-faint">
				Discover & account
			</p>
		...
		<div className="mx-3 mb-1 grid grid-cols-3 gap-1 rounded-xl bg-surface-2 p-1">
			...
			>
				{voiceOn ? (
					...
					<MicOff className="h-[17px] w-[17px]" aria-hidden="true" />
				)}
			</button>
			<button
				type="button"
	...
	return (
		<>
			...
			<aside
				...
				className="fixed inset-y-0 left-0 z-40 hidden w-[200px] flex-col border-r border-line bg-rail lg:flex"
			>
			...
			{navOpen && (
				...
				>
					<div
						className="anim-fade absolute inset-0 bg-black/70"
		...
		>
			{MOBILE_NAV.map(({ id, label, icon: Icon }) => {
				...
				return (
					...
					>
						{active && (
							<span className="absolute inset-x-6 top-0 h-[2px] rounded-full bg-gold" />
						)}
			...
			<div className="ml-auto flex min-w-0 flex-1 items-center justify-end gap-2.5 md:gap-3">
				{searchEnabled && (
					<div
						className={cn(
							...
							focused
								? "border-gold/60 shadow-[var(--glow-gold)]"
								: "border-line",
						)}
					>
				...
				>
					{profile.avatar_url ? (
						...
						<span className="grid h-full w-full place-items-center bg-gold text-[13px] font-bold text-black">
							{initial}
						</span>
					)}
				</button>
