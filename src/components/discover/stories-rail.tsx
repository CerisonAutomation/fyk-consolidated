...
type StoryItem = {
  ...
  background: string | null; created_at: string; viewed: boolean;
};
type Ring = { author: ProfileUser; items: StoryItem[] };
...
export function StoriesRail() {
  ...
  const [viewing, setViewing] = useState<Ring | null>(null);
  const [idx, setIdx] = useState(0);
  const [composing, setComposing] = useState(false);

  const { data, isLoading } = useQuery({
    queryKey: ["stories"],
    queryFn: () => api<{ mine: StoryItem[]; rings: Ring[] }>("/api/stories"),
  });

  const markViewed = useMutation({
    mutationFn: (storyId: string) =>
      api("/api/stories", { method: "PUT", body: { storyId } }),
  });

  const create = useMutation({
    mutationFn: (body: { media_url: string; caption?: string; background?: string }) =>
      api("/api/stories", { method: "POST", body }),
    onSuccess: () => {
      setComposing(false);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      if (idx + 1 < viewing.items.length) setIdx(idx + 1);
  ...
  return (
    <>
      <div className="mb-5 flex gap-3 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        >
          <div className="relative">
            <Avatar name={me?.pseudo ?? "You"} photoUrl={me?.photos?.[0]} size={64} />
        ...
        {rings.map((r) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className="rounded-full border-2 border-ink">
                  <Avatar name={r.author.pseudo} photoUrl={r.author.photos?.[0]} size={60} />
                </div>
              </div>
              <span className={cn("max-w-16 truncate text-[10px]", hasNew ? "text-white" : "text-muted")}>
                {r.author.pseudo}
              </span>
            </button>
          );
          ...
          <div className="absolute inset-x-4 top-4 z-10 flex gap-1">
            {viewing.items.map((_, i) => (
              <div key={i} className="h-0.5 flex-1 overflow-hidden rounded-full bg-white/20">
                <div
                  className={cn("h-full bg-white transition-all", i < idx && "w-full", i === idx && "w-1/2")}
                />
              </div>
            ))}
          </div>
          ...
          <div className="relative h-full max-h-[85vh] w-full max-w-sm overflow-hidden rounded-3xl">
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-ink to-transparent p-5">
              <div className="flex items-center gap-2.5">
                <Avatar name={viewing.author.pseudo} photoUrl={viewing.author.photos?.[0]} size={34} />
                <div>
                  <p className="text-sm font-semibold text-white">{viewing.author.pseudo}</p>
                  <p className="text-[11px] text-white/60">
                    {viewing.items[idx]?.created_at
                      ? new Date(viewing.items[idx].created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
                      : ""}
                  </p>
                </div>
              </div>
            ...
            <button
              ...
              onClick={() =>
                idx + 1 < viewing.items.length ? setIdx(idx + 1) : setViewing(null)
              }
            />
          </div>
        </div>
      ...
      {/* composer */}
      {composing && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-ink/85 p-4" onClick={() => setComposing(false)}>
          <div className="w-full max-w-sm rounded-2xl border border-line bg-surface p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <StoryComposer onSubmit={(v) => create.mutate(v)} busy={create.isPending} />
          </div>
        </div>
      )}
    </>
...
function StoryComposer({
  onSubmit, busy,
}: { onSubmit: (v: { media_url: string; caption?: string; background?: string }) => void; busy: boolean }) {
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-3 gap-2">
        {PRESETS.map((p) => (
          <button
            key={p.label}
            onClick={() => { setUrl(p.url); setBg(p.bg); }}
            className={cn(
      ...
      <div className="flex flex-wrap gap-1.5">
        {Object.keys(BG).map((k) => (
          <button
            key={k}
            onClick={() => setBg(k)}
            className={cn("h-7 w-7 rounded-full border-2", bg === k ? "border-white" : "border-transparent")}
