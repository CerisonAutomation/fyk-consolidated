...
import { Cpu, Crown, Heart, ShieldCheck, Star, Zap } from "lucide-react";
		...
		>
			{photo ? (
				...
				<div
					...
					style={{ background: gradient(candidate.pseudo) }}
				>
					<span className="text-5xl font-bold text-white/80">
						{candidate.pseudo[0]?.toUpperCase()}
					</span>
				</div>
			)}
			...
			>
				<Heart
					className={cn("h-4 w-4", candidate.isFavorite && "fill-rose-400")}
				/>
			</button>
			...
			<div className="absolute inset-x-0 bottom-0 p-2.5 pr-14">
				...
				<p className="truncate text-[11px] text-white/70">
					{candidate.age ?? "—"} · {candidate.distanceKm}km ·{" "}
					{candidate.geo?.city ?? "Nearby"}
				</p>
				<div className="mt-1 flex gap-1 overflow-hidden">
					{candidate.tribes.slice(0, 2).map((t) => (
						<span
							key={t}
							className="truncate rounded-full bg-white/12 px-1.5 py-0.5 text-[9px] text-white/85 backdrop-blur"
						>
							{t}
						</span>
					))}
			...
			<button
				...
				className={cn(
					...
					isTapped
						...
						: "bg-gold text-ink hover:scale-110 hover:bg-gold-soft",
				)}
