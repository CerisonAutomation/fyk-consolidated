...
import { Badge, Button, Skeleton, Spinner } from "@/components/ui/primitives";
...
export function ProfileModal({
	...
	const { data: analysis } = useQuery({
		...
		queryFn: () =>
			api<{
				...
				method: "POST",
				body: { action: "profileAnalysis", targetId: candidate.id },
			}),
		enabled: tab === "ai",
	});
	...
	async function loadIcebreakers() {
		...
		try {
			const r = await api<{ icebreakers: string[] }>("/api/ai", {
				method: "POST",
				body: { action: "icebreakers", targetId: candidate.id },
			});
			setIcebreakers(r.icebreakers);
		} catch {
			pushToast("AI is warming up — try again", "error");
		} finally {
			setLoadingIce(false);
		}
	}
	...
	async function loadDates() {
		try {
			...
			pushToast("Could not plan dates", "error");
		}
	}
	...
	const reportMutation = useMutation({
		mutationFn: (reason: string) =>
			api("/api/safety/reports", {
				method: "POST",
				body: { reportedId: candidate.id, reason },
			}),
		onSuccess: () => {
			pushToast("Report submitted. Our team reviews 24/7.", "info");
			setShowReport(false);
		},
	});
	...
	const noteMutation = useMutation({
		mutationFn: () =>
			api("/api/notes", {
				method: "POST",
				body: { targetId: candidate.id, content: note },
			}),
		onSuccess: () => {
	...
	return (
		<div
			className="fixed inset-0 z-50 flex items-end justify-center bg-ink/85 backdrop-blur-sm sm:items-center sm:p-4"
			onClick={onClose}
		>
			<div
					...
					<div className="absolute inset-x-0 bottom-0 p-4">
						<div className="flex items-end justify-between gap-3">
							<div className="min-w-0">
								...
								<div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted">
									...
									<span className="flex items-center gap-1">
										...
										{candidate.distanceKm}km · {candidate.geo?.city}
									</span>
									{candidate.height && (
										<span className="flex items-center gap-1">
							...
							<div className="shrink-0 text-right">
								<div className="text-3xl font-bold leading-none text-gradient-gold">
					...
					{tab === "about" && (
						<>
							<p className="text-sm leading-relaxed text-white/90">
								{candidate.description || "No bio yet."}
							</p>
							...
							<div className="grid grid-cols-2 gap-2 text-xs">
								{candidate.bodyType && (
									<Detail
										...
										label={candidate.bodyType}
									/>
								)}
								{candidate.occupation && (
									<Detail
										icon={<Briefcase className="h-3.5 w-3.5" />}
										label={candidate.occupation}
									/>
								)}
								{candidate.languages.length > 0 && (
									<Detail
										icon={<Languages className="h-3.5 w-3.5" />}
										label={candidate.languages.join(", ")}
									/>
								)}
								{candidate.relationshipStatus && (
									<Detail
										icon={<Heart className="h-3.5 w-3.5" />}
										label={candidate.relationshipStatus}
									/>
								)}
								<Detail
									icon={<Shield className="h-3.5 w-3.5" />}
									label={`Trust ${candidate.trustScore}/100`}
								/>
							</div>
						...
						<>
							...
							<div className="flex items-start gap-2 rounded-2xl border border-gold/20 bg-gold/[0.06] p-3">
								...
								<p className="text-xs leading-relaxed text-white/80">
									Based on shared interests, goals, tribes and age proximity.
									Recalculated whenever either of you updates a profile.
								</p>
							</div>
						</>
					)}
						...
						<>
							...
							<Section
								...
								action={
									<button
										...
										disabled={loadingIce}
										className="text-xs text-gold hover:text-gold-soft disabled:opacity-50"
									>
							...
							>
								{loadingIce ? (
											...
											<button
												...
												className={cn(
													...
													copied === msg
														...
														: "border-line bg-surface text-white/80 hover:border-gold/30",
												)}
											>
												<span>{msg}</span>
									...
									<p className="text-xs text-muted">
										Generate 5 openers personalised to {candidate.pseudo}&apos;s
										profile.
									</p>
								)}
							</Section>
							...
							>
								{dateIdeas ? (
									...
									<p className="text-xs text-muted">
										Date ideas matched to their interests and your budget.
									</p>
								)}
							</Section>
				...
				<div className="shrink-0 border-t border-line p-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
						...
						<button
							...
							className={cn(
								...
								candidate.isFavorite
									...
									: "border-line bg-surface-2 text-muted hover:text-rose-400",
							)}
						>
							<Heart
								className={cn(
									"h-4 w-4",
									candidate.isFavorite && "fill-rose-400",
								)}
							/>
						</button>
						<button
					...
					{showNote && (
						<div className="mt-3 rounded-xl border border-gold/25 bg-gold/[0.06] p-3">
							<p className="mb-2 text-xs font-medium text-gold-soft">
								Private note about {candidate.pseudo}
							</p>
							<textarea
								value={note}
