...
import { BODY_TYPES, LOOKING_FOR, MEETNOW, TRIBES } from "@/lib/constants";
...
export function DiscoverClient() {
	...
	const tapMutation = useMutation({
		...
		onSuccess: (res, targetId) => {
			...
			pushToast(
				res.isMatch ? "It's a match! ✨ You tapped each other" : "Tap sent 👑",
				res.isMatch ? "info" : "success",
			);
	...
	const favMutation = useMutation({
		mutationFn: (targetId: string) =>
			api<{ favorited: boolean }>("/api/social", {
				method: "POST",
				body: { action: "favorite", targetId },
			}),
		onSuccess: (res) => {
			pushToast(
				res.favorited ? "Added to favorites ❤️" : "Removed from favorites",
	...
	const candidates = useMemo(() => {
		return all
			.filter((c) => {
				if (filters.tribe !== "All" && !c.tribes.includes(filters.tribe))
					return false;
				if (
					filters.looking !== "All" &&
					!c.lookingFor.includes(filters.looking)
				)
					return false;
				if (filters.body !== "All" && c.bodyType !== filters.body) return false;
				if (filters.intent !== "All" && !c.intents.includes(filters.intent))
					return false;
				if (c.age && (c.age < filters.minAge || c.age > filters.maxAge))
					return false;
	...
	const activeFilterCount =
		(filters.tribe !== "All" ? 1 : 0) +
		(filters.looking !== "All" ? 1 : 0) +
		(filters.body !== "All" ? 1 : 0) +
		...
		<div>
					...
					{vectorScores.size > 0 && (
						<>
							...
							<span className="text-purple-300">
								Vector search boosted {vectorScores.size} profiles.
							</span>
						</>
					)}
			...
			<div className="mb-4 flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
				...
				{TRIBES.slice(0, 6).map((t) => (
					<Chip
						key={t}
						active={filters.tribe === t}
						onClick={() =>
							setFilters((f) => ({ ...f, tribe: f.tribe === t ? "All" : t }))
						}
					>
						{t}
					</Chip>
				...
				{(meta?.vibes ?? []).slice(0, 3).map((v) => (
					<Chip
						key={v.vibe}
						active={filters.intent === v.vibe}
						onClick={() =>
							setFilters((f) => ({
								...f,
								intent: f.intent === v.vibe ? "All" : v.vibe,
							}))
						}
					>
						{v.vibe} energy
			...
			{selected && (
				<ProfileModal
					...
					onMessage={async () => {
						const r = await api<{ conversationId: string }>(
							"/api/conversations",
							{
								method: "POST",
								body: { targetId: selected.id },
							},
						);
						navigate({
...
function MapView({
	...
	me: { geo: { lat: number; lng: number; city: string } | null } | null;
}) {
	...
	if (withGeo.length === 0)
		return (
			<EmptyState
				...
				description="Nobody nearby has location enabled."
			/>
		);
			...
			>
				...
				<div className="space-y-5">
					...
					<Select
						...
						options={["All", ...LOOKING_FOR]}
						onChange={(v) => set("looking", v)}
					/>
					<Select
						label="Body type"
						value={filters.body}
