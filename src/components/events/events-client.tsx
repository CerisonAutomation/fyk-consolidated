...
export function EventsClient() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const payload = await api<Record<string, unknown>>(`/api/events?${params.toString()}`);
      return {
        items: listOf<Record<string, unknown>>(payload, "events").map((row) => ({
          id: String(row.id ?? ""),
          name: typeof row.name === "string" ? row.name : undefined,
          description: typeof row.description === "string" ? row.description : undefined,
          // `attendee_count` is the only membership number an event has.
          members: typeof row.attendee_count === "number" ? row.attendee_count : undefined,
          createdAt: typeof row.start_time === "string" ? row.start_time : undefined,
        })) as EventsItem[],
  ...
  const boostMutation = useEntityAction("activity", "boost", {
    invalidate: ["events"],
    onDone: () => vibrate(20),
    onRefused: setActionNote,
  });
  const engageMutation = useEntityAction("activity", "engage", {
    invalidate: ["events"],
    onDone: () => vibrate(10),
    onRefused: setActionNote,
  });
    ...
    return (
      <div className="mx-auto max-w-3xl p-4">
        <div className="rounded-[20px] border border-red-200 bg-red-50 p-6 text-center">
          <p className="text-[14px] font-medium text-red-800">Failed to load events</p>
    ...
    <div className="mx-auto max-w-5xl p-4 pb-24">
      ...
      <div className="mb-6 rounded-[20px] border border-black/[0.06] bg-white/80 p-5 shadow-[0_1px_2px_rgba(0,0,0,0.04),0_4px_12px_rgba(0,0,0,0.06)] backdrop-blur-xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="font-display text-[30px] font-bold tracking-[-0.02em] text-black capitalize">events</h1>
            <p className="mt-1 text-[14px] text-zinc-500">Events with RSVP, calendar, location, vs Grindr events</p>
      ...
      {data?.items.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-zinc-200 bg-zinc-50 p-12 text-center">
          <p className="text-[14px] font-medium text-zinc-700">No events found</p>
              ...
              {viewMode === "grid" ? (
                <>
                  ...
                  <div className="absolute inset-x-0 bottom-0 z-10 p-3">
                    <p className="font-display text-[16px] font-semibold leading-tight text-white">{item.name ?? item.title ?? "events " + item.id.slice(0, 4)}</p>
                    <p className="mt-0.5 line-clamp-2 text-[12px] leading-[1.3] text-white/70">{item.description ?? "Events with RSVP, calendar, location, vs Grindr events"}</p>
                    ...
                    <div className="mt-2 flex gap-1.5">
                      ...
                      >
                        ...
                        {boostMutation.isPending
                          ? "Working…"
                          : entityLabel("activity", "boost") ?? "Boost"}
                      </button>
                      <button type="button"
                      ...
                      >
                        ...
                        {entityLabel("activity", "engage") ?? "Like"}
                      </button>                    </div>
                  </div>
                </>
              ) : (
      ...
      <div className="mt-8 rounded-[16px] border bg-zinc-50 p-4">
        <h3 className="flex items-center gap-2 text-[13px] font-semibold text-black">
          <Crown className="h-4 w-4" /> events — production patterns
        </h3>
        <ul className="mt-2 grid gap-1.5 text-[11px] leading-[1.4] text-zinc-600 md:grid-cols-2">
          <li>• Real API: `/api/events` with Drizzle ORM, RLS, rate limiting</li>
          ...
          <li>• vs Grindr: Events with RSVP, calendar, location, vs Grindr events</li>
