...
export function EventsClient() {
	...
	const { data, isLoading } = useQuery({
		queryKey: ["events"],
		queryFn: () =>
			api<{ events: EventItem[] }>("/api/events").then((r) => r.events),
	});
	...
	const rsvp = useMutation({
					...
					events: prev.events.map((e) =>
						e.id === eventId
							? {
									...
									attendee_count:
										(e.attendee_count ?? 0) + (action === "join" ? 1 : -1),
								}
							: e,
					),
				});
		...
		onSuccess: (_d, vars) => {
			pushToast(
				vars.action === "join" ? "You're going! 🎉" : "RSVP cancelled",
				vars.action === "join" ? "success" : "info",
			);
	...
	const upcoming = events.filter(
		(e) => new Date(e.start_time ?? 0).getTime() >= now,
	);
	...
	const list =
		tab === "upcoming"
			? upcoming
			: tab === "mine"
				? events.filter((e) => e.isMine || e.attending)
				: past;
		...
		<div>
			...
			<div className="mb-4 flex gap-1.5">
				{(
					[
						...
						[
							"mine",
							`My events (${events.filter((e) => e.isMine || e.attending).length})`,
						],
			...
			{isLoading ? (
				...
				<EmptyState
					...
					description={
						tab === "mine"
							? "RSVP to something or host your own."
							: "Check back soon — the community is always planning."
					}
					action={
						tab === "mine" ? (
								...
								<div className="flex flex-col sm:flex-row sm:items-stretch">
									<div className="flex w-full shrink-0 items-center justify-center gap-2 border-b border-line bg-surface-2 px-5 py-3 sm:w-24 sm:flex-col sm:border-b-0 sm:border-r">
										<span className="text-2xl font-bold text-gold">
											{e.start_time ? format(new Date(e.start_time), "d") : "—"}
										</span>
									...
									>
										...
										<div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-muted">
											<span className="flex items-center gap-1">
												...
												{e.start_time
													? format(new Date(e.start_time), "EEE h:mm a")
													: "TBD"}
											</span>
											<span className="flex items-center gap-1">
											...
											<span className="flex items-center gap-1">
												...
												{e.attendee_count ?? 0}
												{e.max_attendees ? `/${e.max_attendees}` : ""} going
											</span>
											{e.cost && (
			...
			{detail && (
				<EventDetail
					...
					onRsvp={(a) => rsvp.mutate({ eventId: detail.id, action: a })}
				/>
			)}
...
function EventDetail({
	...
	event: EventItem;
	onClose: () => void;
	onRsvp: (action: "join" | "leave") => void;
}) {
	const full = event.max_attendees
	...
	const countdown = event.start_time
		? Math.max(
				0,
				Math.round(
					(new Date(event.start_time).getTime() - Date.now()) / 86400000,
				),
			)
		: 0;
		...
		>
			...
			>
				<div
					...
					style={{ background: gradient(event.name) }}
				>
					<button
						onClick={onClose}
					...
					<span className="absolute bottom-3 left-4 text-4xl">
						{CAT_EMOJI[event.category ?? "Other"] ?? "✨"}
					</span>
				</div>
				<div className="p-5">
					...
					<Button
						...
						onClick={() => {
							onRsvp(event.attending ? "leave" : "join");
							onClose();
						}}
				...
				{step === 1 && (
					<div className="space-y-3">
						...
						<button
							...
							className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-gold/30 bg-gold/10 py-2 text-xs font-medium text-gold-soft"
						>
					...
					<div className="space-y-3">
						...
						<div className="rounded-xl border border-gold/20 bg-gold/[0.06] p-3">
							<p className="flex items-start gap-1.5 text-[11px] text-white/80">
								...
								Thursday and Sunday evenings see the highest RSVP rates in your
								area.
							</p>
						</div>
					</div>
				...
				{step === 3 && (
					<div className="space-y-3">
						<MapSearchBar
							...
							onSelect={(feature) =>
								setForm((f) => ({ ...f, location: feature.place_name }))
							}
						/>
						<input
							value={form.location}
					...
					<div className="space-y-3">
						<div className="rounded-2xl border border-line bg-surface-2 p-4">
							...
							<div className="mt-3 space-y-1 text-[11px] text-muted">
								<p>
									...
									{form.date || "Date TBD"} at {form.time}
								</p>
								<p>
								...
								<p>
									...
									Max {form.max_attendees} · ${form.cost}
								</p>
							</div>
						</div>
						<Button
						...
						{!valid && (
							<p className="text-center text-[11px] text-muted">
								Add a title, description and location to publish.
							</p>
						)}
					</div>
				)}
