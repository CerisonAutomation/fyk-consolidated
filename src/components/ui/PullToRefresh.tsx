 ...
 */
export function PullToRefresh({ onRefresh, children, className, disabled = false }: { onRefresh: () => Promise<void>; children: ReactNode; className?: string; disabled?: boolean }) {
	...
	const onTouchStart = useCallback(
		(event: React.TouchEvent) => {
			...
			const root = (event.target as HTMLElement).closest("[data-pull-root]") ?? (event.currentTarget as HTMLElement);
	...
	return (
		<div data-pull-root className={cn("relative touch-pan-y", className)} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={() => void onTouchEnd()} onTouchCancel={() => { startY.current = null; setPull(0); }}>
			<div className="pointer-events-none absolute inset-x-0 top-0 flex h-0 items-center justify-center" style={{ height: Math.max(0, pull) }} aria-hidden={!refreshing && pull === 0}>
				<span
					...
					style={{ transform: `rotate(${(pull / MAX_PULL) * 270}deg) scale(${refreshing ? 1 : 0.7 + (pull / MAX_PULL) * 0.3})` }}
				>
					<RefreshCw className="h-4 w-4" />
				</span>
			</div>
			<div className={cn("transition-transform duration-150", pull > 0 && !refreshing && "duration-0")} style={{ transform: `translateY(${pull}px)` }}>
				{children}
			</div>
		</div>
	);
