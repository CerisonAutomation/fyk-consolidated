...
export function Panel({
  ...
  return (
    <Tag className={cn("rounded-2xl border border-line bg-surface shadow-[var(--shadow-card)]", className)}>
      {children}
    </Tag>
  );
}
...
export function DataModeNotice({
  ...
  return (
    ...
    >
      ...
      <p className={cn("leading-relaxed", compact ? "text-[11.5px]" : "text-[12.5px]")}>
        <strong className="font-semibold text-violet">{title}.</strong> {children}
      </p>
    </div>
  );
}
