...
export function StateBlock({
	...
	const Icon = kind === "loading" ? Loader2 : kind === "error" || kind === "disabled" ? AlertTriangle : Inbox;
	return (
		<div
			role={kind === "error" ? "alert" : "status"}
			aria-busy={kind === "loading"}
			className={cn("flex flex-col items-center justify-center rounded-2xl border border-dashed border-line bg-surface/50 px-6 py-14 text-center", className)}
		>
			<span
				className={cn(
					"mb-4 grid h-12 w-12 place-items-center rounded-2xl",
					kind === "error" ? "bg-live/10 text-live" : kind === "disabled" ? "bg-white/5 text-muted" : "bg-gold/10 text-gold",
				)}
			>
...
export function describeFailure(error: unknown, fallback = "We couldn't load that right now."): { message: string; retryable: boolean; requestId: string | null } {
	if (error instanceof ApiClientError) {
		return { message: error.message, retryable: error.retryable || error.status >= 500, requestId: error.requestId };
	}
