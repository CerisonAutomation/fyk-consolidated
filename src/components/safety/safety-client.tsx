...
const TIPS = [
	{
		...
		desc: "Chat on FYK until you genuinely trust someone. Off-platform requests in the first few messages are a scam signal.",
	},
	{
		icon: Eye,
		title: "Video verify first",
		desc: "Look for the verified badge and suggest a quick video call before meeting in person.",
	},
	{
		icon: Smartphone,
		title: "Meet in public",
		desc: "First meetings belong in busy public places. Share your live location with a friend.",
	},
	{
		icon: Flag,
		title: "Report anything",
		desc: "If something feels off, report it. Our moderation mesh reviews reports around the clock.",
	},
	{
		icon: FileLock2,
		title: "Consent media rails",
		desc: "Explicit media requires both people to opt in first. Every image is watermarked and revocable.",
	},
	{
		icon: Handshake,
		title: "Set the pact early",
		desc: "Agree on what you're both looking for inside the chat -- before expectations mismatch.",
	},
];
...
function profileDisplay(p: SafetyProfile | null) {
	...
	return {
		...
		online:
			!p.hide_online &&
			Date.now() - new Date(p.last_active_at).getTime() < 5 * 60_000,
	};
}
	...
	return (
		<div className="mx-auto max-w-2xl">
			...
			<p className="mb-4 text-sm text-muted">
				Your safety is non-negotiable. Manage visibility, blocks and private
				notes.
			</p>
			...
			{isIncognito && (
				<div className="mb-4 flex items-center gap-2 rounded-2xl border border-purple-500/30 bg-purple-500/10 p-3">
					...
					<p className="text-xs text-white/85">
						...
						You browse without appearing in anyone's footprints or grid.
					</p>
				</div>
			)}
			...
			<div className="mb-5 rounded-2xl border border-line bg-surface p-4">
				...
				<p className="mb-3 text-[11px] text-muted">
					Share your approximate location with a trusted contact before meeting
					someone new.
				</p>
				<button
					onClick={() =>
						pushToast("Location shared with your emergency contact", "success")
					}
			...
			{tab === "viewed" ? (
				loadingFp ? (
					...
					<EmptyState
						...
						description="As people discover you, views show up here (Gold feature)."
					/>
				) : (
					<div className="space-y-2">
						{footprints.map((f) => {
							...
							if (!dp) return null;
							const meta =
								PRESET_META[f.preset ?? "visited"] ?? PRESET_META.visited;
			...
			tab === "blocked" ? (
				loadingBlocked ? (
					...
					<EmptyState
						...
						description="Block anyone from their profile -- instantly and silently."
					/>
				) : (
					<div className="space-y-2">
						{blocked.map((b) => {
							...
							return (
								...
								>
									...
									<div className="min-w-0 flex-1">
										...
										{b.reason && (
											<p className="truncate text-[11px] text-muted capitalize">
												Reason: {b.reason}
											</p>
										)}
									</div>
			...
			loadingNotes ? (
				...
				<EmptyState
					...
					description="Jot down reminders about people you meet -- only you ever see these."
				/>
			) : (
				<div className="space-y-2">
					{notes.map((n) => {
				...
				>
					...
					>
						<div className="mb-3 flex items-center gap-3">
							<Avatar
								name={profileDisplay(noteTarget)?.pseudo ?? ""}
								photoUrl={profileDisplay(noteTarget)?.photoUrl}
								size={40}
							/>
