...
describe("newRequestId", () => {
	it("trusts a well-formed inbound id so a trace survives the proxy chain", () => {
		expect(newRequestId(new Headers({ "x-request-id": "trace-abc_123" }))).toBe(
			"trace-abc_123",
		);
	});
	...
	it("replaces a hostile or malformed id instead of reflecting it", () => {
		expect(
			newRequestId(new Headers({ "x-request-id": "<script>x</script>" })),
		).toMatch(/^fyk_[a-f0-9]{20}$/);
		expect(newRequestId(new Headers({ "x-request-id": "ab" }))).toMatch(
			/^fyk_/,
		);
...
describe("readJson", () => {
	...
	it("returns parsed data for a valid body", async () => {
		await expect(
			readJson(request({ name: "Nik", age: 31 }), schema),
		).resolves.toEqual({ name: "Nik", age: 31 });
	});

	it("surfaces a single field error as the authored message", async () => {
		const error = await readJson(request({ name: "N", age: 31 }), schema).catch(
			(caught) => caught,
		);
	...
	it("keeps a message the schema author wrote", async () => {
		const authored = z.object({
			handle: z.string().regex(/^[a-z]+$/, "Lowercase letters only."),
		});
		const error = await readJson(request({ handle: "NO" }), authored).catch(
			(caught) => caught,
		);
	...
	it("lists every offending field when there is more than one", async () => {
		const error = await readJson(request({ name: "N", age: 5 }), schema).catch(
			(caught) => caught,
		);
	...
	it("refuses junk, wrong content types and oversized bodies", async () => {
		await expect(readJson(request("{not json"), schema)).rejects.toThrow(
			/not valid JSON/,
		);
		await expect(
			readJson(
				new Request("http://x/api/y", { method: "POST", body: "hi" }),
				schema,
			),
		).rejects.toThrow(/Content-Type/);
		await expect(
			readJson(
				request({ blob: "x".repeat(70 * 1024) }),
	...
	it("applies schema defaults instead of trusting the client to send them", async () => {
		...
		await expect(
			readJson(request({ body: "hi" }), withDefault),
...
describe("readQuery", () => {
	it("coerces and validates query strings", () => {
		...
		expect(readQuery(new URLSearchParams("page=2&when=past"), schema)).toEqual({
			page: 2,
			when: "past",
		});
		expect(readQuery(new URLSearchParams(), schema)).toEqual({
			page: 0,
			when: "upcoming",
		});
		expect(() =>
			readQuery(new URLSearchParams("when=whenever"), schema),
		).toThrow(/filters are not valid/);
	});
});
