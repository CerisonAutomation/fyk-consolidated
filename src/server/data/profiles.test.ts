...
import { ACTIVE_WINDOW_MINUTES, ONLINE_WINDOW_MINUTES, affinity, availableNow, coarsen, distanceBetween, encodeGeohash, formatDistance, presenceOf } from "./profiles";
...
describe("presenceOf", () => {
	it("reads presence from the last write, never from a pushed event", () => {
		...
		expect(presenceOf({ hide_online: false, last_active_at: new Date(now - 60_000).toISOString() }, now)).toBe("online");
		expect(presenceOf({ hide_online: false, last_active_at: new Date(now - (ONLINE_WINDOW_MINUTES + 1) * 60_000).toISOString() }, now)).toBe("active");
		expect(presenceOf({ hide_online: false, last_active_at: new Date(now - (ACTIVE_WINDOW_MINUTES + 1) * 60_000).toISOString() }, now)).toBe("offline");
	});

	it("reports offline for an unparseable timestamp and for hide_online", () => {
		expect(presenceOf({ hide_online: false, last_active_at: "not a date" }, Date.now())).toBe("offline");
		expect(presenceOf({ hide_online: true, last_active_at: minutesAgo(0) }, Date.now())).toBe("offline");
	});
});

describe("availableNow", () => {
	it("expires: an availability window in the past is not 'open'", () => {
		expect(availableNow({ open_to_meet: true, available_until: minutesAgo(5) })).toBe(false);
		expect(availableNow({ open_to_meet: true, available_until: minutesAgo(-60) })).toBe(true);
	});

	it("treats an unset window as open and a false flag as closed", () => {
		expect(availableNow({ open_to_meet: true, available_until: null })).toBe(true);
		expect(availableNow({ open_to_meet: false, available_until: minutesAgo(-30) })).toBe(false);
	});
});
...
describe("distanceBetween", () => {
	...
	it("returns nothing when the member hid their distance", () => {
		const result = distanceBetween(viewer, { lat_coarse: 35.91, lng_coarse: 14.5, hide_distance: true }, "v", "t");
		expect(result).toEqual({ km: null, label: null });
	});

	it("returns nothing when either side has no coarsened fix", () => {
		expect(distanceBetween(null, { lat_coarse: 35.9, lng_coarse: 14.5, hide_distance: false }, "v", "t").km).toBeNull();
		expect(distanceBetween(viewer, { lat_coarse: null, lng_coarse: 14.5, hide_distance: false }, "v", "t").km).toBeNull();
	});

	it("gives the same number to the same viewer/target pair and hides sub-grid precision", () => {
		const target = { lat_coarse: 35.9123, lng_coarse: 14.5011, hide_distance: false };
		...
		expect(Math.abs((other.km ?? 0) - (first.km ?? 0))).toBeLessThanOrEqual(0.6);
	});
});
...
describe("coarsen + encodeGeohash", () => {
	...
	it("matches a known geohash vector", () => {
		...
		expect(encodeGeohash(35.8997, 14.5147, 4).slice(0, 2)).toBe(encodeGeohash(35.8998, 14.5148, 4).slice(0, 2));
		expect(encodeGeohash(0, 0, 5)).toBe("s0000");
	});

	it("separates far-apart points", () => {
		expect(encodeGeohash(35.9, 14.5, 5)).not.toBe(encodeGeohash(52.52, 13.4, 5));
	});

	it("uses only the base32 alphabet", () => {
		expect(encodeGeohash(-33.86, 151.21, 9)).toMatch(/^[0-9bcdefghjkmnpqrstuvwxyz]{9}$/);
	});
});

describe("affinity", () => {
	it("counts shared tags case-insensitively and caps the score", () => {
		const result = affinity(["Music", "FITNESS"], { interests: ["music", "fitness", "film"], looking_for: ["dating"] });
