...
import { z } from "zod";
import { badRequest, conflict, notFound, dbFailure } from "../errors";
import { readJson, type RequestCtx } from "../context";

export const REPORT_REASONS = ["harassment", "spam", "fake_profile", "inappropriate_content", "underage", "threat", "doxxing", "other"] as const;
/** Anything that alleges a minor, a threat, or doxxing goes straight to urgent. */
export const URGENT_REASONS: readonly string[] = ["underage", "threat", "doxxing"];
...
export const reportSchema = z.object({
	targetType: z.enum(["profile", "message", "board_post", "event", "album"]).default("profile"),
	targetId: z.string().uuid(),
	reason: z.enum(REPORT_REASONS),
	details: z.string().trim().max(1000).optional(),
	/** "Are you sure?" is captured as data so the queue can weigh an accidental tap. */
	confirmed: z.boolean().refine((value) => value === true, { message: "Confirm the report before sending." }),
});
...
export async function submitReport(ctx: RequestCtx) {
	...
	if (body.targetType === "profile" && body.targetId === caller.userId) throw badRequest("You cannot report your own profile.");

	const target =
		body.targetType === "profile"
			? await client.from("profiles").select("id,is_suspended").eq("id", body.targetId).maybeSingle()
			: null;
	...
	const open = await client
		...
		.limit(1);
	if ((open.data ?? []).length) throw conflict("You already reported this for that reason. A moderator is on it.");
...
export async function myReports(ctx: RequestCtx) {
	...
	const { data, error } = await client
		.from("reports")
		.select("id,target_type,target_id,reason,status,severity,created_at,reviewed_at")
		.eq("reporter_id", caller.userId)
		.order("created_at", { ascending: false })
		.limit(50);
	if (error) throw dbFailure(error, "That did not save. Please try again.");
	return {
		reports: ((data ?? []) as unknown as Record<string, unknown>[]).map((row) => ({
			id: row.id,
...
export async function blockUser(ctx: RequestCtx) {
	const caller = await ctx.auth();
	const body = await readJson(ctx.request, z.object({ targetId: z.string().uuid(), reason: z.enum(["harassment", "spam", "fake", "other"]).optional() }));
	const client = ctx.db();
	void body.reason; // Blocks are unconditional; a reason would invite appeal threads.

	if (body.targetId === caller.userId) throw badRequest("You cannot block yourself.");

	const existing = await client.from("blocks").select("id").eq("blocker_id", caller.userId).eq("blocked_id", body.targetId).maybeSingle();
	if (existing.data) throw conflict("You already blocked that person.");

	const insert = await client.from("blocks").insert({ blocker_id: caller.userId, blocked_id: body.targetId });
	if (insert.error) throw dbFailure(insert.error, "That did not save. Please try again.");
	...
	const conversations = await client.from("conversation_members").select("conversation_id").eq("profile_id", caller.userId);
	const ids = ((conversations.data ?? []) as unknown as { conversation_id: string }[]).map((row) => row.conversation_id);
	if (ids.length) {
		const others = await client.from("conversation_members").select("conversation_id,profile_id").in("conversation_id", ids).eq("profile_id", body.targetId);
		const toArchive = ((others.data ?? []) as unknown as { conversation_id: string }[]).map((row) => row.conversation_id);
		if (toArchive.length) {
			await client
...
export async function unblockUser(ctx: RequestCtx, targetId: string) {
	...
	const client = ctx.db();
	const remove = await client.from("blocks").delete().eq("blocker_id", caller.userId).eq("blocked_id", targetId);
...
export async function blockList(ctx: RequestCtx) {
	const caller = await ctx.auth();
	const client = ctx.db();
	const blocks = await client.from("blocks").select("blocked_id,created_at").eq("blocker_id", caller.userId).order("created_at", { ascending: false }).limit(200);
	if (blocks.error) throw dbFailure(blocks.error, "That did not save. Please try again.");
	const ids = ((blocks.data ?? []) as unknown as { blocked_id: string }[]).map((row) => row.blocked_id);
	if (!ids.length) return { blocked: [] };
	const profiles = await client.from("profiles").select("id,display_name,handle,avatar_url").in("id", ids);
	const map = new Map(((profiles.data ?? []) as unknown as { id: string; display_name: string | null; handle: string | null; avatar_url: string | null }[]).map((row) => [row.id, row]));
	return {
		blocked: ids.map((id) => ({
			id,
			displayName: map.get(id)?.display_name ?? map.get(id)?.handle ?? "Blocked member",
...
export async function whoViewedMe(ctx: RequestCtx) {
	...
	const visits = (data ?? []) as unknown as { visitor_id: string; viewed_at: string }[];
	...
	if (profiles.error) throw dbFailure(profiles.error, "We could not load that list.");
	type Viewer = { id: string; display_name: string | null; handle: string | null; avatar_url: string | null; age: number | null };
	const byId = new Map(((profiles.data ?? []) as unknown as Viewer[]).map((row) => [row.id, row]));
