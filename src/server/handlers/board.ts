...
import { badRequest, conflict, dbFailure, forbidden, notFound } from "../errors";
import { readJson, type RequestCtx } from "../context";

const POST_COLUMNS = "id,author_id,kind,body,activity_id,city,area,spots,join_count,expires_at,created_at";
...
export async function listBoard(ctx: RequestCtx) {
	...
	if (kind && !["invite", "offer", "ask", "text"].includes(kind)) throw badRequest("Unknown Board filter.");
	...
	if (postsResult.error) throw dbFailure(postsResult.error, "That did not save. Please try again.");

	const rows = (postsResult.data ?? []) as unknown as Record<string, unknown>[];
	if (!rows.length) return { posts: [], note: "The Board is quiet right now. Post first — people respond to something specific." };
	...
	const [profilesResult, myJoinsResult, allJoinsResult] = await Promise.all([
		...
		client.from("post_joins").select("post_id").in("post_id", postIds).eq("profile_id", caller.userId),
		client.from("post_joins").select("post_id,profile_id").in("post_id", postIds),
	]);

	const profileMap = new Map(((profilesResult.data ?? []) as unknown as ProfileRow[]).map((row) => [row.id, row]));
	const joined = new Set(((myJoinsResult.data ?? []) as unknown as { post_id: string }[]).map((row) => row.post_id));
	const joiners = new Map<string, string[]>();
	for (const row of (allJoinsResult.data ?? []) as unknown as { post_id: string; profile_id: string }[]) {
		const list = joiners.get(row.post_id) ?? [];
		list.push(row.profile_id);
		joiners.set(row.post_id, list);
	}

	const viewer = rows.length ? await client.from("profiles").select(PROFILE_LIST_COLUMNS).eq("id", caller.userId).maybeSingle() : { data: null };
	...
	const out = rows.map((row) => {
		...
		const people = (joiners.get(String(row.id)) ?? [])
			...
			.filter(Boolean)
			.slice(0, 6)
			.map((profile) => ({ id: profile!.id, displayName: profile!.display_name ?? profile!.handle ?? "Someone", avatarUrl: profile!.avatar_url }));
		return {
			id: row.id,
			kind: row.kind,
			body: row.body,
			...
			author: author
				? toPublicProfile(author, {
						viewer: viewerRow
							? { id: caller.userId, lat: viewerRow.lat_coarse, lng: viewerRow.lng_coarse, interests: viewerRow.interests ?? [] }
							: null,
						client,
					})
				: null,
...
export async function createBoardPost(ctx: RequestCtx) {
	...
	const existing = await client.from("board_posts").select("id").eq("author_id", caller.userId).gt("expires_at", new Date().toISOString()).limit(1);
	if ((existing.data ?? []).length) throw conflict("You already have a live post. Update or remove it first.");

	const expiresAt = new Date(Date.now() + body.windowMinutes * 60_000).toISOString();
	...
	await client
		.from("profiles")
		.update({ open_to_meet: true, available_until: expiresAt, last_active_at: new Date().toISOString() })
		.eq("id", caller.userId);
...
export async function deleteBoardPost(ctx: RequestCtx, postId: string) {
	...
	const client = ctx.db();
	const owner = await client.from("board_posts").select("id,author_id").eq("id", postId).maybeSingle();
	if (!owner.data) throw notFound("That post is gone.");
	if ((owner.data as { author_id: string }).author_id !== caller.userId) throw forbidden("You can only remove your own post.");
	const remove = await client.from("board_posts").delete().eq("id", postId).eq("author_id", caller.userId);
...
export async function setBoardJoin(ctx: RequestCtx, postId: string) {
	...
	if (body.joining) {
		const insert = await client.from("post_joins").insert({ post_id: postId, profile_id: caller.userId });
		// The trigger raises post_full / post_expired / owner_cannot_join; those
		// names are mapped to friendly text by mapUnknownError.
		if (insert.error) throw dbFailure(insert.error, "That did not save. Please try again.");
	} else {
		const remove = await client.from("post_joins").delete().eq("post_id", postId).eq("profile_id", caller.userId);
		if (remove.error) throw dbFailure(remove.error, "That did not save. Please try again.");
	}

	const after = await client.from("board_posts").select("join_count,spots,expires_at").eq("id", postId).maybeSingle();
	return {
		joined: body.joining,
		joinCount: (after.data as { join_count?: number } | null)?.join_count ?? null,
	};
}
