...
import { type ApiClient, readJson } from "../context";
import {
	encodeGeohash,
...
export const discoverQuerySchema = z.object({
	geohash: z
		.string()
		.regex(/^[0-9a-z]{4,9}$/, "Location precision is not valid.")
		.optional(),
...
export async function getViewerRow(
	ctx: RequestCtx,
...
export async function listNearby(ctx: RequestCtx) {
	...
	const prefix =
		query.geohash ??
		(viewer.lat_coarse != null
			? encodeGeohash(viewer.lat_coarse, viewer.lng_coarse ?? 0, Math.min(6, 5))
			: undefined);
	...
	const maxKm = query.maxKm;
	const viewerLat = viewer.lat_coarse;
	const viewerLng = viewer.lng_coarse;
	if (maxKm && viewerLat != null && viewerLng != null) {
		rows = rows.filter((row) => {
			if (row.hide_distance || row.lat_coarse == null || row.lng_coarse == null)
				return true;
			return (
				haversineKm(
					{ lat: viewerLat, lng: viewerLng },
					{ lat: row.lat_coarse, lng: row.lng_coarse },
				) <= maxKm
			);
		});
	}
	...
	const candidates = rows
		...
		.filter((candidate) =>
			query.hasPhotos ? candidate.photos.length > 0 : true,
		)
		.filter((candidate) =>
			query.maxKm && candidate.distanceKm != null
				? candidate.distanceKm <= query.maxKm
				: true,
		);
...
export async function getProfile(ctx: RequestCtx, profileId: string) {
	...
	const { data, error } = await client
		.from("profiles")
		.select(
			`${PROFILE_LIST_COLUMNS},bio,age_verified_at,incognito,is_suspended`,
		)
		.eq("id", profileId)
		.maybeSingle();
	if (error) throw dbFailure(error, "That did not save. Please try again.");
	...
	const profile = toPublicProfile(row, {
		viewer: {
			id: caller.userId,
			lat: viewer?.lat_coarse ?? null,
			lng: viewer?.lng_coarse ?? null,
...
async function readRelationship(
	...
	const mine = (likes.data ?? []).find(
		(row: { from_id: string }) => row.from_id === meId,
	);
	const theirs = (likes.data ?? []).find(
		(row: { from_id: string }) => row.from_id === otherId,
	);
...
export async function tap(ctx: RequestCtx) {
	...
	if (body.targetId === caller.userId)
		throw badRequest("You cannot tap yourself.");
	...
	if ((blocked.data ?? []).length)
		throw forbidden("You cannot interact with this person.");

	if (body.action === "pass" || body.action === "unfavorite") {
		await client
			.from("likes")
			.delete()
			.eq("from_id", caller.userId)
			.eq("to_id", body.targetId);
	...
	if (existing.data) {
		...
		const insert = await client
			.from("likes")
			.insert({ from_id: caller.userId, to_id: body.targetId, kind });
		if (insert.error)
