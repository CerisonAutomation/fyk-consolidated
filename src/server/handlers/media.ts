...
import { z } from "zod";
import { badRequest, dbFailure, forbidden, notFound, ApiFailure } from "../errors";
import { publicUrl, type ProfileRow } from "../data/profiles";
import { readJson, type RequestCtx } from "../context";

const MAX_BYTES = 12 * 1024 * 1024;
const IMAGE_TYPES = new Set(["image/jpeg", "image/png", "image/webp"]);
const CHAT_TYPES = new Set([...IMAGE_TYPES, "video/mp4", "video/webm", "audio/webm", "audio/mpeg", "audio/mp4"]);
...
export async function uploadPhotos(ctx: RequestCtx) {
	...
	try {
		for (const [index, file] of files.entries()) {
			if (!IMAGE_TYPES.has(file.type)) throw badRequest(`Only JPEG, PNG or WebP photos are supported (got ${file.type || "unknown type"}).`);
			if (file.size > MAX_BYTES) throw badRequest(`Each photo must be under ${Math.round(MAX_BYTES / 1024 / 1024)} MB.`);

			const path = `${caller.userId}/${Date.now()}-${index}.${extensionFor(file.type)}`;
			const upload = await client.storage.from("photos-public").upload(path, file, { contentType: file.type, cacheControl: "31536000", upsert: false });
			if (upload.error) throw dbFailure(upload.error, "That did not save. Please try again.");
			uploaded.push(path);
		}
	} catch (error) {
		// Never leave orphaned objects behind when a later photo fails.
		if (uploaded.length) await client.storage.from("photos-public").remove(uploaded);
		throw error;
	}
	...
	const insert = await client.from("profile_photos").insert(rows).select("id,storage_path,bucket,position,is_primary");
	if (insert.error) throw dbFailure(insert.error, "That did not save. Please try again.");

	await client
		.from("profiles")
		.update({ avatar_url: publicUrl(client, "photos-public", uploaded[0]), last_active_at: new Date().toISOString() })
		.eq("id", caller.userId);

	return {
		photos: ((insert.data ?? []) as unknown as { storage_path: string; bucket: string }[]).map((row) => ({
...
export async function uploadAvatar(ctx: RequestCtx) {
	...
	if (!IMAGE_TYPES.has(file.type)) throw badRequest("Use a JPEG, PNG or WebP image.");
	...
	const upload = await client.storage.from("avatars-public").upload(path, file, { contentType: file.type, cacheControl: "31536000", upsert: true });
	if (upload.error) throw dbFailure(upload.error, "That did not save. Please try again.");

	const update = await client.from("profiles").update({ avatar_url: publicUrl(client, "avatars-public", path) }).eq("id", caller.userId).select("id,avatar_url").single();
	if (update.error) throw dbFailure(update.error, "That did not save. Please try again.");
	return { avatarUrl: (update.data as unknown as { avatar_url: string }).avatar_url };
}

export async function touchPresence(ctx: RequestCtx) {
	const caller = await ctx.auth();
	const client = ctx.db();
	const { error } = await client.from("profiles").update({ last_active_at: new Date().toISOString() }).eq("id", caller.userId);
	if (error) throw dbFailure(error, "That did not save. Please try again.");
	const me = await client.from("profiles").select("id,last_active_at,open_to_meet,available_until").eq("id", caller.userId).maybeSingle();
	return { at: new Date().toISOString(), state: (me.data ?? null) as unknown as Pick<ProfileRow, "last_active_at"> | null };
}
...
export async function uploadChatMedia(ctx: RequestCtx) {
	...
	if (!/^[0-9a-f-]{36}$/i.test(conversationId)) throw badRequest("Which conversation is this for?");

	const type = file.type || "application/octet-stream";
	if (!CHAT_TYPES.has(type)) throw new ApiFailure("unsupported_media", "FYK chat accepts JPEG, PNG, WebP, MP4, WebM or M4A files.");
	const limit = type.startsWith("video/") ? 32 * 1024 * 1024 : 12 * 1024 * 1024;
	if (file.size > limit) throw new ApiFailure("payload_too_large", type.startsWith("video/") ? "Videos must be under 32 MB." : "Images must be under 12 MB.");
	...
	const upload = await client.storage.from("chat-media-private").upload(path, file, { contentType: type, cacheControl: "3600", upsert: false });
	if (upload.error) throw dbFailure(upload.error, "That upload failed. Try again.");

	const { data: signed } = await client.storage.from("chat-media-private").createSignedUrl(path, 600);

	return {
		storagePath: path,
		kind: type.startsWith("video/") ? "video" : type.startsWith("audio/") ? "audio" : "image",
...
export async function listMyPhotos(ctx: RequestCtx) {
	...
	const { data, error } = await client
		.from("profile_photos")
		.select("id,storage_path,bucket,position,is_primary,width,height,created_at")
		.eq("owner_id", caller.userId)
		.order("position", { ascending: true });
	if (error) throw dbFailure(error, "We could not load your photos.");
	const rows = (data ?? []) as unknown as { id: string; storage_path: string; bucket: string | null; position: number; is_primary: boolean; width: number | null; height: number | null; created_at: string }[];
	return {
		photos: rows.map((row) => ({
			id: row.id,
...
export async function deletePhotos(ctx: RequestCtx) {
	const caller = await ctx.auth();
	const body = await readJson(ctx.request, z.object({ ids: z.array(z.string().uuid()).min(1).max(9) }));
	const client = ctx.db();
	...
	const existing = await client.from("profile_photos").select("id,storage_path,bucket,is_primary").eq("owner_id", caller.userId).in("id", body.ids);
	if (existing.error) throw dbFailure(existing.error, "That did not save. Please try again.");
	const rows = (existing.data ?? []) as unknown as { id: string; storage_path: string; bucket: string | null; is_primary: boolean }[];
	if (!rows.length) throw notFound("None of those photos are on your profile.");

	const remove = await client.from("profile_photos").delete().eq("owner_id", caller.userId).in("id", rows.map((row) => row.id));
	if (remove.error) throw dbFailure(remove.error, "That did not save. Please try again.");
	...
	const remaining = await client.from("profile_photos").select("id,storage_path,bucket,position").eq("owner_id", caller.userId).order("position", { ascending: true }).limit(1);
	const primary = (remaining.data ?? [])[0] as { storage_path: string; bucket: string | null } | undefined;
	await client
		.from("profiles")
		.update({ avatar_url: primary ? publicUrl(client, primary.bucket ?? "photos-public", primary.storage_path) : null, last_active_at: new Date().toISOString() })
		.eq("id", caller.userId);
