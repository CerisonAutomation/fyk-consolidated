...
import { type ApiFailure, badRequest } from "../errors";
...
const probeColumns = {
	...
	profilePhotos:
		"id,owner_id,storage_path,bucket,position,is_primary,width,height",
	boardPosts:
		"id,author_id,kind,body,expires_at,created_at,join_count,spots,activity_id",
...
async function probe(
	client: ApiClient,
	table: keyof typeof probeColumns,
): Promise<ApiFailure | null> {
	const { error } = await client
		.from(table)
		.select(probeColumns[table])
		.limit(1);
...
export async function getSession(ctx: RequestCtx): Promise<SessionResponse> {
	...
	const needsOnboarding =
		!row?.onboarding_completed_at || !row?.age_verified_at;
	const self =
		row?.lat_coarse != null && row.lng_coarse != null
			? {
					lat: row.lat_coarse,
					lng: row.lng_coarse,
					geohash6: encodeGeohash(row.lat_coarse, row.lng_coarse),
				}
			: null;
	...
	return {
		...
		capabilities: {
			...capabilities,
			moderation: caller.role !== "user" && !reportsErr,
		},
		schemaIssues,
	};
}
...
export function adultAgeOrThrow(dob: string, now = new Date()): number {
	...
	if (Number.isNaN(birth.getTime()))
		throw badRequest("That date of birth is not valid.");
