...
export function dependencyUnavailable(
	message = "That feature is not available yet.",
) {
...
export function dbFailure(
	...
	switch (error.code) {
		case "23505":
			return new ApiFailure("conflict", "You already have that.", {
				sqlstate: error.code,
			});
		case "23503":
			return new ApiFailure("bad_request", "That record no longer exists.", {
				sqlstate: error.code,
			});
		case "23514":
			return new ApiFailure(
				"bad_request",
				"Some of that is not allowed here.",
				{ sqlstate: error.code },
			);
		case "42501":
			return new ApiFailure("forbidden", "You cannot do that.", {
				sqlstate: error.code,
			});
		case "P0001":
			// A raised exception from one of our own functions: its message is authored.
			return new ApiFailure("bad_request", redact(error.message ?? fallback), {
				sqlstate: error.code,
			});
		default:
			return new ApiFailure("internal_error", fallback, {
...
const SECRET_PATTERNS: Array<[RegExp, string]> = [
	...
	// Supabase keys
	[
		/\b(sb_(?:publishable|secret|anon)_[A-Za-z0-9_-]{10,}|eyJ[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,})\b/g,
		REDACTION,
	],
	...
	[
		/\b(?:avatars-public|chat-media-private|albums-private|photos-public|event-media-public)\/[^\s"']+/g,
		REDACTION,
	],
...
export function mapUnknownError(
	...
	switch (pg) {
		...
		case "23503":
			return {
				code: "bad_request",
				message: `The ${context} references something that no longer exists.`,
			};
		case "23514":
			return {
				code: "bad_request",
				message: `Those values are not allowed for ${context}.`,
			};
		case "42501":
			return {
				code: "forbidden",
		...
		case "22P02":
			return {
				code: "bad_request",
				message: `That ${context} is not in the right format.`,
			};
		case "PGRST116":
	...
	const rules: Array<[RegExp, ErrorCode, string]> = [
		[
			/not authorized|jwt expired|invalid jwt|invalid claim/i,
			"unauthorized",
		...
		[
			/message_identity_is_immutable|too late to edit|too late to recall|edit window|recall window/i,
			"forbidden",
			"That message can no longer be changed.",
		],
		[
			/pin limit|too many pinned/i,
			"conflict",
			"You have reached the pin limit for this conversation.",
		],
...
export function failureResponse(
	...
	if (failure.code === "rate_limited")
		headers["Retry-After"] = String(failure.retryAfterSeconds ?? 60);
	...
	if (failure.retryable || failure.code === "rate_limited")
