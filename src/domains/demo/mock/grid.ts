import { DEMO_ID_START, DEMO_PROFILE_COUNT, GRID_PAGE_SIZE } from "../config";
import {
	type DemoSeed,
	distanceForId,
	lastOnlineOf,
...
export function setDemoFavorite(profileId: number, favorite: boolean): void {
	if (favorite) {
		demoFavoriteSet.add(profileId);
	} else {
		demoFavoriteSet.delete(profileId);
	}
}
	...
	return ids.sort(
		(a, b) =>
			(distances.get(a) ?? Number.POSITIVE_INFINITY) -
				(distances.get(b) ?? Number.POSITIVE_INFINITY) || a - b,
	);
})();
