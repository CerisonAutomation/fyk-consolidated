...
describe("textMessageSchema", () => {
	...
	it("rejects text message with empty text", () => {
		const result = textMessageSchema.safeParse({
			type: "Text",
			body: { text: 123 },
		});
...
describe("messageSchema (discriminated union)", () => {
	...
	it("rejects unknown message types that don't match any variant", () => {
...
describe("outboundMessageSchema", () => {
	...
	it("rejects outbound message with extra body fields not in schema", () => {
		const result = outboundMessageSchema.safeParse({
			type: "Text",
			body: { text: "hi", extra: "not allowed" },
		});
		// Zod strict mode would reject, but default is passthrough
